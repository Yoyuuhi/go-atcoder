...
func main() {
	...
	mem[1] = (1 + 2 + 3 + 4 + 5 + 6) / 6.0
	ans := mem[1]
	for i := 1; i < n; i++ {
		mem[i+1] = (maxFloat(1, mem[i]) + maxFloat(2, mem[i]) + maxFloat(3, mem[i]) + maxFloat(4, mem[i]) + maxFloat(5, mem[i]) + maxFloat(6, mem[i])) / 6.0
		ans = maxFloat(ans, mem[i+1])
	}
