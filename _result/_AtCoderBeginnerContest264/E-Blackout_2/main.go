...
func main() {
	...
	edges := make([]edge, e)
	for i := 0; i < e; i++ {
		edges[i] = edge{from: ni() - 1, to: ni() - 1}
	}
	q := ni()
	xs := make([]int, q)
	mapXs := map[int]bool{}
	...
	memGroupWithElect := map[int]bool{}
	uf := newUnionFind(n + m)
	tempAns := 0
	for i, edge := range edges {
		if edge.from >= n {
			memGroupWithElect[edge.from] = true
		}
		if edge.to >= n {
			memGroupWithElect[edge.to] = true
		}

		if !mapXs[i] {
			if memGroupWithElect[uf.root(edge.from)] || memGroupWithElect[uf.root(edge.to)] {
				switch false {
				case memGroupWithElect[uf.root(edge.from)]:
					tempAns += uf.size(edge.from)
				case memGroupWithElect[uf.root(edge.to)]:
					tempAns += uf.size(edge.to)
				}
				memGroupWithElect[uf.root(edge.from)] = true
				memGroupWithElect[uf.root(edge.to)] = true
			}
