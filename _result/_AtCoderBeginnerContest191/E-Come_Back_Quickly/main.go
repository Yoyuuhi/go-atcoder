...
func main() {
	...
	edges := make([][]edge, n)
	froms := map[int][]int{}
	for a, bc := range memEs {
		for b, c := range bc {
			froms[b] = append(froms[b], a)
			edges[a] = append(edges[a], edge{to: b, cost: c})
		}
	}
	g := newgraph(n, edges)
	...
	for i := 0; i < n; i++ {
		dist := g.dijkstra(i)
		for _, j := range froms[i] {
			if dist[j] != inf {
				ans[i] = min(ans[i], dist[j]+memEs[j][i])
			}
		}
	}
