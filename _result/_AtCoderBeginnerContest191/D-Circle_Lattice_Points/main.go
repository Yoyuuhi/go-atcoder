...
func main() {
	...
	a, b, c := nf(), nf(), nf()
	x, y, R = int(math.Round(a*10000)), int(math.Round(b*10000)), int(math.Round(c*10000))
	out(calc())
}
...
func calc() int {
	...
	for i := ceil10000(y - R); i <= floor10000(y+R); i += 10000 {
		h := abs(i - y)
		...
		for (x-l)*(x-l)+h*h > R*R && l <= x {
			l += 10000
		}
		for (r-x)*(r-x)+h*h > R*R && r >= x {
			r -= 10000
		}
...
func ceil10000(x int) int {
	return int(math.Ceil(float64(x)/10000) * 10000)
}
func floor10000(x int) int {
	return int(math.Floor(float64(x)/10000) * 10000)
}
