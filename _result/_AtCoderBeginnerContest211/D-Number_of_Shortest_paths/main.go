...
func main() {
	...
	passed := map[int]passMem{}
	e := cities.Front()
	passed[0] = passMem{cost: 0, count: 1}
	added := map[int]bool{}
	for e != nil {
		now := e.Value.(int)

		for _, edge := range edges[now] {
			if _, e := passed[edge.to]; !e {
				passed[edge.to] = passMem{cost: inf, count: 0}
			}

			if passed[edge.to].cost < passed[now].cost+1 {
				continue
			}
			// out(now, edge.to, passed[edge.to].count+passed[now].count)
			passed[edge.to] = passMem{
				cost:  passed[now].cost + 1,
				count: (passed[edge.to].count + passed[now].count) % mod1000000007,
			}
