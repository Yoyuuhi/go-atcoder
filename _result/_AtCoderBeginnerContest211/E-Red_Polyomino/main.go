...
func simulate(count int, mem [][]string) {
	s := memToS(mem)
	if passed[s] {
		return
	}
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			...
			for _, v := range vectorsA {
				toX := i + v.x
				toY := j + v.y
				if count != k && (toX < 0 || toX >= n) {
					continue
				}
				if count != k && (toY < 0 || toY >= n) {
					continue
				}
				if count == k || mem[toX][toY] == "@" {
					connected = true
				}
			}
			if !connected {
			...
			mem[i][j] = "@"
			simulate(count-1, mem)
			mem[i][j] = "."
		}
	}
}

func memToS(mem [][]string) string {
	ans := ""
	for _, ss := range mem {
		ans += strings.Join(ss, "")
	}
