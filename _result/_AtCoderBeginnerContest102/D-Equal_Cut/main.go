...
func main() {
	...
	ans := inf
	for j := 1; j+1 < n-1; j++ {
		i := bs(0, j-1, func(c int) bool {
			return cm.get(c) <= cm.getRange(c+1, j)
		})
		i = max(i, 0)
		k := bs(j+1, n-2, func(c int) bool {
			return cm.getRange(j+1, c) <= cm.getRange(c+1, n-1)
		})
		k = max(k, j+1)

		if i+1 < j && cm.get(i+1)-cm.getRange(i+2, j) < cm.getRange(i+1, j)-cm.get(i) {
			i++
		}
		if k+1 < n-1 && cm.getRange(j+1, k+1)-cm.getRange(k+2, n-1) < cm.getRange(k+1, n-1)-cm.getRange(j+1, k) {
			k++
		}

		mi1 := min(cm.get(i), cm.getRange(i+1, j))
		mi2 := min(cm.getRange(j+1, k), cm.getRange(k+1, n-1))
		ma1 := max(cm.get(i), cm.getRange(i+1, j))
		ma2 := max(cm.getRange(j+1, k), cm.getRange(k+1, n-1))
		ans = min(ans, max(ma1, ma2)-min(mi1, mi2))
	}
	out(ans)
}
