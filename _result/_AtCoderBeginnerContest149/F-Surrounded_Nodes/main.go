...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		for _, edge := range edges[i] {
			to := edge.to
			if memFrom[i] != to {
				sum = madd(sum, nodeCount[to])
				now = madd(now, madd(m2n[nodeCount[to]], -1))
			}
		}
		if memFrom[i] != -1 {
			now = madd(now, m2n[n-sum-1]-1)
		}
		cases = madd(cases, m2n[n-1]-1-now)
	}
	out(mdiv(cases, m2n[n]))
}

func graphDfs(edges [][]edge, now int, passed map[int]bool) int {
	res := 0
	for _, edge := range edges[now] {
		if !passed[edge.to] {
			passed[edge.to] = true
			memFrom[edge.to] = now
			res = madd(res, graphDfs(edges, edge.to, passed))
		}
	}
	nodeCount[now] = madd(res, 1)
	return nodeCount[now]
}
