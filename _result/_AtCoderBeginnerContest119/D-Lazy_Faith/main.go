...
func main() {
	...
	for i := 0; i < q; i++ {
		x := ni()
		si := bs(0, len(ss)-1, func(c int) bool {
			return ss[c] <= x
		})
		ti := bs(0, len(ts)-1, func(c int) bool {
			return ts[c] <= x
		})
		out(calc(x, si, ti, ss, ts))
	}
}

func calc(x, si, ti int, ss, ts []int) int {
	sx := []int{}
	tx := []int{}
	...
	if si+1 < len(ss) {
		sx = append(sx, ss[si+1])
	}
	if ti >= 0 {
		tx = append(tx, ts[ti])
	}
	if ti+1 < len(ts) {
		tx = append(tx, ts[ti+1])
	}
	...
	for _, s := range sx {
		for _, t := range tx {
			...
			ans = min(ans, 2*(x-l)+r-x)
			ans = min(ans, x-l+2*(r-x))
		}
	}
	return ans
}
