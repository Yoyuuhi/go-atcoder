...
func simulate(x, y int, mem [][]bool, count int) {
	if count == a {
		ans++
		return
	}
	...
	if x+1 < h && !mem[x+1][y] {
		mem[x][y] = true
		mem[x+1][y] = true
		x1, y1, _ := next(x, y, mem)
		simulate(x1, y1, mem, count+1)
		mem[x][y] = false
		mem[x+1][y] = false
	}
	if y+1 < w && !mem[x][y+1] {
	...
	x1, y1, _ = next(x1, y1, mem)
	simulate(x1, y1, mem, count)
}

func next(x, y int, mem [][]bool) (int, int, bool) {
	for y < w && mem[x][y] {
		x++
		if x == h {
			x = 0
