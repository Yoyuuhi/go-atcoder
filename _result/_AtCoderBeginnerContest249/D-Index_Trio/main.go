...
func main() {
	...
	mapAs := make(map[int]int, n)
	for i := 0; i < n; i++ {
		mapAs[as[i]] += 1
	}

	factorMap := make(map[int][]int, len(mapAs))
	for k, _ := range mapAs {
		factorMap[k] = factorize(k, mapAs)
	}

	ans := 0
	for k, v := range factorMap {
		for _, a := range v {
			ans += mapAs[k] * mapAs[a] * mapAs[k/a]
		}
	}
	out(ans)
}

func factorize(n int, mapAs map[int]int) []int {
	ans := make([]int, 0)
