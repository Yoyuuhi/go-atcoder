...
func main() {
	...
	dp, sum := make([][]int, n+2), make([][]int, n+2)
	for i := 0; i < n+2; i++ {
		dp[i], sum[i] = make([]int, n+2), make([]int, n+2)
	}

	for i := 1; i <= n; i++ {
		sum[i][0] = mmul(mpow(25, p-2, p), 26)
	}
	...
	for i := 1; i <= n; i++ {
		for j := 1; j <= n; j++ {
			for k := 0; k < 4; k++ {
				...
				x := max(i - tens[k] + 1, 0)
				y := max(i - tens[k+1] + 1, 0)
				dp[i][j] = madd(dp[i][j], mmul(madd(sum[x][j-l-1], -sum[y][j-l-1]), 25))
			}
			sum[i+1][j] = madd(sum[i+1][j], madd(sum[i][j], dp[i][j]))
		}
	}
