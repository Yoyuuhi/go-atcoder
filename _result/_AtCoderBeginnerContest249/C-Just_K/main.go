...
func main() {
	...
	n, k := ni2()
	mem := make([]map[string]int, n)
	for i := 0; i < n; i++ {
		mem[i] = make(map[string]int, 26)
		str := ns()
		for j := 0; j < len(str); j++ {
			mem[i][string(str[j])] += 1
		}
	}

	ans := 0
	for bits := 0; bits < (1 << uint64(n)); bits++ {
		res := calcTypes(mem, k, bits)
		ans = max(ans, res)
	}
	out(ans)
}

func calcTypes(mem []map[string]int, k int, bits int) int {
	bitMap := make(map[int]int, len(mem))
	for i := 0; i < len(mem); i++ {
	...
	charNum := make(map[string]int, 26)
	for k1, _ := range bitMap {
		for k2, _ := range mem[k1] {
			charNum[k2] += 1
		}
	}
