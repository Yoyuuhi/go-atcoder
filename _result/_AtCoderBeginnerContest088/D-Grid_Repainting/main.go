...
func main() {
	...
	ans := ps - 1 - wfs(sX, point{0, 0}, point{h - 1, w - 1}, h, w)
	if ans == ps-1 {
		out(-1)
		return
	}
...
func wfs(sX []string, from, to point, h, w int) int {
	l := list.New()
	l.PushBack(from)
	lE := l.Front()

	passed := map[int]map[int]int{}
	passed[from.x] = map[int]int{}
	passed[from.x][from.y] = 0

	for lE != nil {
		now := lE.Value.(point)
		if _, e := passed[now.x+1]; !e {
			passed[now.x+1] = map[int]int{}
		}
		if _, e := passed[now.x-1]; !e {
			passed[now.x-1] = map[int]int{}
		}
		if _, e := passed[now.x]; !e {
			passed[now.x] = map[int]int{}
		}

		if _, e := passed[now.x+1][now.y]; now.x+1 < h && string(sX[now.x+1][now.y]) != "#" && !e {
			passed[now.x+1][now.y] = passed[now.x][now.y] + 1
			l.PushBack(point{x: now.x + 1, y: now.y})
		}
		if _, e := passed[now.x-1][now.y]; now.x-1 >= 0 && string(sX[now.x-1][now.y]) != "#" && !e {
			passed[now.x-1][now.y] = passed[now.x][now.y] + 1
			l.PushBack(point{x: now.x - 1, y: now.y})
		}
		if _, e := passed[now.x][now.y+1]; now.y+1 < w && string(sX[now.x][now.y+1]) != "#" && !e {
			passed[now.x][now.y+1] = passed[now.x][now.y] + 1
			l.PushBack(point{x: now.x, y: now.y + 1})
		}
		if _, e := passed[now.x][now.y-1]; now.y-1 >= 0 && string(sX[now.x][now.y-1]) != "#" && !e {
			passed[now.x][now.y-1] = passed[now.x][now.y] + 1
			l.PushBack(point{x: now.x, y: now.y - 1})
		}
		lE = lE.Next()
	...
	if _, e := passed[to.x]; !e {
		return 0
	}
	return passed[to.x][to.y]
}
