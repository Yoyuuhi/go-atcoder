...
func main() {
	...
	rootNode := map[int][]int{}
	for i := 0; i < n; i++ {
		if uf.size(i) < 3 {
			out(0)
			return
		}
		rootNode[uf.root(i)] = append(rootNode[uf.root(i)], i)
	}

	count := 0
	for _, ns := range rootNode {
		if verify(edges, ns) {
			count++
		} else {
			out(0)
	...
	out(mpow(2, count, mod))
}

func verify(edges [][]edge, ns []int) bool {
	memE := map[int]bool{}
	for _, n := range ns {
