...
func main() {
	...
	var d, e, f int
	d, e, f = calcW(a, x), calcW(b, x), calcW(c, x)
	if d+e+f <= y {
		out("Yes")
		return
	}
	d, e, f = calcW(a, y), calcW(b, y), calcW(c, y)
	if d+e+f <= x {
		out("Yes")
		return
	}
	...
	for i := 0; i < 3; i++ {
		...
		w1 := calcW(abc[(i+1)%3], (x - w))
		w2 := calcW(abc[(i+2)%3], (x - w))
		if w1+w2 <= y {
			out("Yes")
			return
		}
	...
	for i := 0; i < 3; i++ {
		...
		w1 := calcW(abc[(i+1)%3], (y - w))
		w2 := calcW(abc[(i+2)%3], (y - w))
		if w1+w2 <= x {
			out("Yes")
			return
		}
