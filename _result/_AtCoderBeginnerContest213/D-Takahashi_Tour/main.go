...
func main() {
	...
	for i, _ := range edges {
		sort.Slice(edges[i], func(k, l int) bool {
			return edges[i][k].to < edges[i][l].to
		})
	}

	graphDfs(edges, 0, map[int]bool{})
}
...
func graphDfs(edges [][]edge, now int, passed map[int]bool) {
	...
	for i, edge := range edges[now] {
		if !passed[edge.to] {
			graphDfs(edges, edge.to, passed)
			if i == len(edges[now])-1 && now == 0 {
				out(1)
			} else {
				outwoln(now+1, " ")
			}
