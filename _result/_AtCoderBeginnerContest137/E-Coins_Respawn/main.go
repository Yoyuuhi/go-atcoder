...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		s--
		t--
		edges[i] = edge{from: s, to: t, cost: c - p}
	...
	for e != nil {
		now := e.Value.(int)
		for _, edge := range edgesr[now] {
			if memPoss[edge.to] {
				continue
			}
			memPoss[edge.to] = true
	...
	mem[0] = 0
	if BellmanFordMax(mem, edges, n, m, memPoss) == inf {
		out(-1)
		return
	}
...
func BellmanFordMax(mem []int, edges []edge, n, m int, memPoss map[int]bool) int {
	for i := 0; i < n; i++ {
		for j := 0; j < m; j++ {
			e := edges[j]
			if mem[e.from] == -inf {
			...
			if mem[e.to] < mem[e.from]+e.cost {
				mem[e.to] = mem[e.from] + e.cost
				if i == n-1 {
					return inf
				}
			}
