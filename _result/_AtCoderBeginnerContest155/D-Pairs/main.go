...
func main() {
	...
	minus := len(pa) * len(ma)
	zeros := len(za)*(len(pa)+len(ma)) + len(za)*(len(za)-1)/2
	if k <= minus {
		ans := bs(pa[0]*ma[len(ma)-1], pa[len(pa)-1]*ma[0], func(c int) bool {
			count := 0
			for _, a1 := range ma {
				i := bs(len(pa)-1, 0, func(c1 int) bool {
					return pa[c1]*a1 <= c
				})
				if i == -1 {
					continue
	...
	if k > minus && k <= minus+zeros {
		out(0)
		return
	}

	ans := bs(pow(10, 18), 0, func(c int) bool {
		count := 0
		for i := len(ma) - 1; i >= 0; i-- {
			j := bs(len(ma)-1, 0, func(c1 int) bool {
				return ma[c1]*ma[i] <= c
			})
			if j == -1 || j >= i {
				continue
		...
		for i := 0; i < len(pa); i++ {
			j := bs(0, len(pa)-1, func(c1 int) bool {
				return pa[c1]*pa[i] <= c
			})
			if j == -1 || j <= i {
				continue
