...
func main() {
	...
	usedMem[ps[n-1]] = false
	simulate(ps, n-1, usedMem, cps, memCps)
	outis(ps)
}

func simulate(ps []int, ind int, usedMem map[int]bool, cps []int, memCps map[int]int) {
	ansI := -1
	for i := memCps[ps[ind]] + 1; i < len(memCps); i++ {
		if usedMem[memCps[i]] {
			continue
		}
		ansI = i
	...
	offset := 0
	if ansI == -1 {
		usedMem[ps[ind-1]] = false
		simulate(ps, ind-1, usedMem, cps, memCps)
	} else {
		ps[ind] = cps[ansI]
		usedMem[cps[ansI]] = true
		offset++
		for _, p := range cps {
			if !usedMem[p] {
				ps[ind+offset] = p
