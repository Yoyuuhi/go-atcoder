...
func main() {
	...
	ans = false
	wfs(sX, point{posS[0], posS[1]}, h, w, cx)
	if ans {
		out("Yes")
	} else {
		out("No")
...
func wfs(sX [][][]int, start point, h, w int, cx []string) {
	l := list.New()
	l.PushBack(start)
	lE := l.Front()

	sX[start.x][start.y] = []int{0, 0, 0}

	for lE != nil {
		now := lE.Value.(point)

		for i, v := range vectorsA {
			toX := now.x + v.x
			toY := now.y + v.y

			if toX < h && toX >= 0 && toY < w && toY >= 0 && sX[toX][toY][2] != 0 && sX[toX][toY][2] != sX[now.x][now.y][2] {
				ans = true
				return
			}

			if toX < h && toX >= 0 && toY < w && toY >= 0 && sX[toX][toY][2] == 0 && string(cx[toX][toY]) == "." {
				if now.x == start.x && now.y == start.y {
					sX[toX][toY] = []int{now.x, now.y, i + 1}
				} else {
					sX[toX][toY] = []int{now.x, now.y, sX[now.x][now.y][2]}
				}
