...
func main() {
	...
	for i := range ss {
		...
		for j := 0; j < len(ss[i]); j++ {
			if string(ss[i][j]) == "1" {
				...
				edgesb[t] = append(edgesb[t], edge{to: s, cost: 1})
			}
		}
	}
	df = make([]int, n)
	...
	for i := 1; i < n-1; i++ {
		ans := inf
		for s := max(0, i-m+1); s < i; s++ {
			for t := min(n-1, s+1); t <= s+m; t++ {
				if t <= i || string(ss[s][t-s-1]) != "1" {
					continue
				}
				if df[s] == -1 || db[t] == -1 {
					continue
				}
				ans = min(ans, df[s]+db[t]+1)
			}
		}
		if ans == inf {
			anss[i-1] = -1
...
func bfs(edges [][]edge, now int, ans []int) {
	cities := list.New()
	cities.PushBack(now)
	e := cities.Front()
