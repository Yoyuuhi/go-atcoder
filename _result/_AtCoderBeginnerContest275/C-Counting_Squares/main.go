...
func main() {
	...
	passedM = [81][81][81][81]bool{}
	for i := 0; i < len(ps)-1; i++ {
		for j := i + 1; j < len(ps); j++ {
			ans += calc(sX, ps[i], ps[j], i, j, ps)
		}
	}
	out(ans)
}

func calc(sX []string, a, b point, i, j int, ps []point) int {
	v := point{-(b.y - a.y), b.x - a.x}

	p1 := point{a.x + v.x, a.y + v.y}
	p2 := point{b.x + v.x, b.y + v.y}

	ans := 0
	if poss(p1) && poss(p2) {
		s1 := string(sX[p1.x][p1.y])
		s2 := string(sX[p2.x][p2.y])
		if s1 == "#" && s2 == "#" {
			k := findInd(ps, p1)
			l := findInd(ps, p2)
			...
			sel := []int{i, j, k, l}
			sorti(sel)
			if !passedM[sel[0]][sel[1]][sel[2]][sel[3]] {
				ans++
			}
			passedM[sel[0]][sel[1]][sel[2]][sel[3]] = true
		}
	}
...
func poss(p point) bool {
	return p.x >= 0 && p.x < 9 && p.y >= 0 && p.y < 9
}

func findInd(ps []point, p point) int {
	ans := -1
	for i, po := range ps {
