...
var memDiv map[int]map[int]int
	...
	for i := 0; i < k; i++ {
		...
		for j, v := range dp {
			for k1 := 1; k1 <= m; k1++ {
				...
				tempDp[to] = madd(tempDp[to], mdiv(v, m))
			}
		}
		dp = tempDp
	}
...
func mdiv(a, b int) int {
	if _, e1 := memDiv[a]; e1 {
		if v, e2 := memDiv[a][b]; e2 {
			return v
		}
	}

	a %= mod
	ans := a * minvfermat(b, mod) % mod

	if _, e1 := memDiv[a]; !e1 {
		memDiv[a] = map[int]int{}
		memDiv[a][b] = ans
	}
	return ans
}
