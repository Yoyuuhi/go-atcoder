...
func main() {
	...
	dp := make([]int, len(t)+1)
	memC := make([]map[string]int, len(s))
	for i := 0; i < len(s); i++ {
		memC[i] = map[string]int{}
	}
	for i := len(s) - 1; i >= 0; i-- {
		c := string(s[i])
		if i < len(s)-1 {
			for k, v := range memC[i+1] {
				memC[i][k] = v
			}
		}

		if v, e := memC[i][c]; !e || v > i {
			memC[i][c] = i
		}
	}

	idx := memC[0][string(t[0])]
	dp[1] = 1
	...
	out(dp[len(t)]*len(s) - (len(s) - idx - 1))
}
