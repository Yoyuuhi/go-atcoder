...
func main() {
	...
	// out(edges)
	graphDfs(edges, 0, map[int]bool{}, memNV, 0)
	outwoln(memNV[0])
	for i := 1; i < n; i++ {
		outwoln(" ", memNV[i])
	}
...
func graphDfs(edges [][]edge, now int, passed map[int]bool, memNV map[int]int, val int) {
	passed[now] = true
	if memNV[now] != 0 {
		memNV[now], val = memNV[now]+val, memNV[now]+val
	} else {
		memNV[now] = val
	}
	...
	for _, edge := range edges[now] {
		if !passed[edge.to] {
			graphDfs(edges, edge.to, passed, memNV, val)
		}
	}
}
