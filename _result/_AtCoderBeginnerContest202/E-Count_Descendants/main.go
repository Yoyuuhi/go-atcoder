...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if len(levelInout[d]) == 0 {
			out(0)
			continue
		}

		lr := bs(len(levelInout[d])-1, 0, func(c int) bool {
			return levelInout[d][c] >= l
		})
		rl := bs(0, len(levelInout[d])-1, func(c int) bool {
			return levelInout[d][c] <= r
		})
		if lr == -1 || rl == -1 || lr > rl {
			out(0)
			continue
...
func graphDfs(edges [][]edge, now int, level int) {
	inout[now][0] = count
	count++

	for _, edge := range edges[now] {
		graphDfs(edges, edge.to, level+1)
	}
	inout[now][1] = count
	count++
	levelInout[level] = append(levelInout[level], inout[now][0])
}
