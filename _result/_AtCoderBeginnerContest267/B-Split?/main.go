...
func main() {
	...
	for i := 0; i < 10; i++ {
		as[i], _ = strconv.Atoi(string(ns[i]))
	}

	mapColNum = make(map[int][]int, 5)
	mapColNum[0] = []int{6}
	mapColNum[1] = []int{3}
	mapColNum[2] = []int{7, 1}
	mapColNum[3] = []int{4, 0}
...
func downCols(as []int) []int {
	result := make([]int, 7)
colLoop:
	for k, v := range mapColNum {
		for _, num := range v {
			if as[num] != 0 {
				continue colLoop
			}
