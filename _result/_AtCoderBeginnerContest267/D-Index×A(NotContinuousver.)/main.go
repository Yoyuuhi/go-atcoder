...
func main() {
	...
	dp[0][0] = big.NewInt(int64(0))
	dp[0][1] = big.NewInt(int64(as[0]))

	for i := 1; i < n; i++ {
		dp[i][0] = big.NewInt(int64(0))
	}
	...
	for i := 1; i < n; i++ {
		for j := 1; j <= i+1 && j <= m; j++ {
			...
			y.Add(dp[i-1][j-1], big.NewInt(int64(j*as[i])))
			if dp[i-1][j] == nil {
				dp[i-1][j] = big.NewInt(int64(-1_000_000_000_000_000_000))
			}
