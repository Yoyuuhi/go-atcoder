...
func main() {
	...
	posSumMap := map[int]int{}
	for pos, es := range edges {
		sum := 0
		for _, e1 := range es {
			sum += as[e1.to]
		}
	...
	for k, v := range posSumMap {
		heap.Push(pq, pqst{x: v, y: k})
	}
	...
	for pq.Len() > 0 {
		...
		for _, edge := range edges[pos] {
			if !passed[edge.to] {
				posSumMap[edge.to] -= as[pos]
				heap.Push(pq, pqst{x: posSumMap[edge.to], y: edge.to})
			}
		}
	}
	out(ans)
