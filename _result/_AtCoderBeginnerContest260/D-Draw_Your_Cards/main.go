...
func main() {
	...
	for i := 1; i < n+1; i++ {
		...
		minAvaiTop := inf
		addStackI := -1
		for iStack, stack := range dp {
			if stack[len(stack)-1] > ps[i-1] && stack[len(stack)-1] < minAvaiTop {
				minAvaiTop = stack[len(stack)-1]
				addStackI = iStack
			}
		}
		...
		if addStackI == -1 {
			...
			stack[0] = ps[i-1]
			dp = append(dp, stack)
		} else if len(dp[addStackI])+1 < k {
			dp[addStackI] = append(dp[addStackI], ps[i-1])
		} else {
			ans[ps[i-1]-1] = i
			for _, v := range dp[addStackI] {
				ans[v-1] = i
			}
			dp = append(dp[:addStackI], dp[addStackI+1:]...)
		}
	}
