...
func main() {
	...
	passMathScores := make(map[int]int, n)
	passMathIds := make(map[int][]int, n)
	for i := 0; i < n; i++ {
		if as[i] >= mathPass {
			passMathScores[as[i]] = 1
			passMathIds[as[i]] = append(passMathIds[as[i]], i+1)
		}
	}
	keys := []int{}
	for k, _ := range passMathScores {
		keys = append(keys, k)
	}
	sorti(keys)
	...
	for i := len(keys) - 1; i >= 0; i-- {
		if len(passMathIds[keys[i]])+count <= x {
			ans = append(ans, passMathIds[keys[i]]...)
			count += len(passMathIds[keys[i]])
		} else {
			sorti(passMathIds[keys[i]])
			ans = append(ans, passMathIds[keys[i]][:x-count]...)
			break
		}
	}
	for i := 0; i < len(ans); i++ {
	...
	// out(ans, 1, mathPass, passMathIds)
	...
	passEngScores := make(map[int]int, n)
	passEngIds := make(map[int][]int, n)
	for i := 0; i < n; i++ {
		if bs[i] >= engPass {
			passEngScores[bs[i]] = 1
			passEngIds[bs[i]] = append(passEngIds[bs[i]], i+1)
		}
	}
	keys = []int{}
	for k, _ := range passEngScores {
		keys = append(keys, k)
	}
	sorti(keys)
	...
	count = 0
	for i := len(keys) - 1; i >= 0; i-- {
		if len(passEngIds[keys[i]])+count <= y {
			ans = append(ans, passEngIds[keys[i]]...)
			count += len(passEngIds[keys[i]])
		} else {
			sorti(passEngIds[keys[i]])
			ans = append(ans, passEngIds[keys[i]][:y-count]...)
			break
		}
	}
	for i := 0; i < len(ans); i++ {
	...
	// out(ans, 2, engPass, passEngIds)

	// sum
	csS := []int{}
	for i := 0; i < n; i++ {
		csS = append(csS, as[i]+bs[i])
	}
	sorti(csS)
	var sumPass int
	if z != 0 {
	...
	passSumScores := make(map[int]int, n)
	passSumIds := make(map[int][]int, n)
	for i := 0; i < n; i++ {
		if sums[i] >= sumPass {
			passSumScores[sums[i]] = 1
			passSumIds[sums[i]] = append(passSumIds[sums[i]], i+1)
		}
	}
	keys = []int{}
	for k, _ := range passSumIds {
		keys = append(keys, k)
	}
	sorti(keys)
	...
	for i := len(keys) - 1; i >= 0; i-- {
		...
		if len(passSumIds[keys[i]])+count <= z {
			ans = append(ans, passSumIds[keys[i]]...)
			count += len(passSumIds[keys[i]])
		} else {
			sorti(passSumIds[keys[i]])
			ans = append(ans, passSumIds[keys[i]][:z-count]...)
			break
		}
	}
	for i := 0; i < len(ans); i++ {
