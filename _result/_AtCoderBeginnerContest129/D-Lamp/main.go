...
func main() {
	...
	for i := 0; i < h; i++ {
		uf := newUnionFind(w)
		for j := 1; j < w; j++ {
			if string(sx[i][j]) == "." && string(sx[i][j-1]) == "." {
				uf.unite(j, j-1)
			}
		}
		for j := 0; j < w; j++ {
			memW[i+1][j+1] = uf.size(j)
		}
	}
	for j := 0; j < w; j++ {
		uf := newUnionFind(h)
		for i := 1; i < h; i++ {
			if string(sx[i][j]) == "." && string(sx[i-1][j]) == "." {
				uf.unite(i, i-1)
			}
		}
		for i := 0; i < h; i++ {
			memH[i+1][j+1] = uf.size(i)
		}
	}
	...
	for i := 1; i <= h; i++ {
		for j := 1; j <= w; j++ {
			if string(sx[i-1][j-1]) == "." {
				ans = max(ans, memH[i][j]+memW[i][j]-1)
			}
		}
	}
	out(ans)
