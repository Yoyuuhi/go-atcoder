...
func main() {
	...
	l := ns()
	dp := make([][2]int, len(l)+1)

	dp[0][1] = 1
	for i := 0; i < len(l); i++ {
		switch string(l[i]) {
		case "0":
			dp[i+1][0] = madd(dp[i+1][0], mmul(2, dp[i][0]))

			dp[i+1][1] = madd(dp[i+1][1], dp[i][1])
			dp[i+1][0] = madd(dp[i+1][0], dp[i][0])
		case "1":
			dp[i+1][1] = madd(dp[i+1][1], mmul(2, dp[i][1]))
			dp[i+1][0] = madd(dp[i+1][0], mmul(2, dp[i][0]))

			dp[i+1][0] = madd(dp[i+1][0], dp[i][1])
			dp[i+1][0] = madd(dp[i+1][0], dp[i][0])
		}
	}
	out(madd(dp[len(l)][0], dp[len(l)][1]))
}
