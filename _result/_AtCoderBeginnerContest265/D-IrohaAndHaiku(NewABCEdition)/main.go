...
func main() {
	...
	for i := 0; i < n; i++ {
		if checkApplicability(sumMem, i, p, q, r) {
			out("Yes")
			return
		}
	}
...
func checkApplicability(sumMem []int, i, p, q, r int) bool {
	iXUpper := bs(i, len(sumMem)-1, func(index int) bool {
		return sumMem[index]-sumMem[i] <= p
	}) - 1
	if sumMem[iXUpper+1]-sumMem[i] != p || iXUpper > len(sumMem)-2 {
		return false
	}

	iYUpper := bs(iXUpper+1, len(sumMem)-1, func(index int) bool {
		return sumMem[index]-sumMem[iXUpper+1] <= q
	}) - 1
	if sumMem[iYUpper+1]-sumMem[iXUpper+1] != q || iYUpper > len(sumMem)-2 {
		return false
	}

	iZUpper := bs(iYUpper+1, len(sumMem)-1, func(index int) bool {
		return sumMem[index]-sumMem[iYUpper+1] <= r
	}) - 1
	if sumMem[iZUpper+1]-sumMem[iYUpper+1] != r || iZUpper > len(sumMem)-1 {
		return false
	}
