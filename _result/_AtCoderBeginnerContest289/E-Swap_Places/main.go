...
func calc() {
	...
	cities := list.New()
	cities.PushBack([]int{0, n - 1})
	e := cities.Front()

	for mem[n-1][0] == inf && e != nil {
		now := e.Value.([]int)

		for _, edge1 := range edges[now[0]] {
			for _, edge2 := range edges[now[1]] {
				to1 := edge1.to
				to2 := edge2.to
				if cs[to1] == cs[to2] {
					continue
				}
				if mem[now[0]][now[1]]+1 >= mem[to1][to2] {
					continue
				}
				mem[to1][to2] = mem[now[0]][now[1]] + 1
				cities.PushBack([]int{to1, to2})
			}
		}
		e = e.Next()
	}
