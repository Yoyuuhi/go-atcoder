...
func main() {
	...
	for i := 0; i < q; i++ {
		x := ni()
		if x < as[0] {
			temp = memM[memS[0]] + n*(as[0]-x)
			out(temp)
			continue
		}
		if x > as[n-1] {
			temp = memM[memS[n-1]] + n*(x-as[n-1])
			out(temp)
			continue
		}

		index := bs(n-1, 0, func(index int) bool {
			return as[index] >= x
		})

		temp = memM[as[index]]
		temp -= index * (as[index] - x)
		temp += (len(as) - index) * (as[index] - x)
		out(temp)
	}
}
