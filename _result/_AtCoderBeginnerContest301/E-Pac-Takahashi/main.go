...
func main() {
	...
	ans := -1
	passed := make([][]int, co+1)
	for i := range passed {
		passed[i] = make([]int, 1<<(co+2))
	}
	passed[0][0] = 0
	cities := list.New()
	cities.PushBack([]int{0, 1, 0})
	e := cities.Front()
	for e != nil {
		now := e.Value.([]int)
		s := now[0]
		...
		for t := 1; t < co+2; t++ {
			if hasbit(bit, t) || mem[s][t] == inf {
				continue
			}
			if passed[s][bit]+mem[s][t] > T {
				continue
			}
			...
			v := passed[t][bit+1<<t]
			if v == 0 {
				passed[t][bit+1<<t] = passed[s][bit] + mem[s][t]
				cities.PushBack([]int{t, bit + 1<<t, ta + 1})
			} else {
				passed[t][bit+1<<t] = min(v, passed[s][bit]+mem[s][t])
			}
		}
		e = e.Next()
	}
