...
func main() {
	...
	for i := 0; i < q; i++ {
		indexA := bs(len(aArr)-1, 0, func(c int) bool {
			return aArr[c] >= bArr[i]
		})

		if indexA == 0 {
			out(abs(aArr[indexA] - bArr[i]))
		} else if indexA == -1 {
			out(abs(aArr[len(aArr)-1] - bArr[i]))
		} else {
			min := abs(aArr[indexA] - bArr[i])
			if abs(aArr[indexA-1]-bArr[i]) < min {
				out(abs(aArr[indexA-1] - bArr[i]))
			} else {
				out(min)
			}
		}
