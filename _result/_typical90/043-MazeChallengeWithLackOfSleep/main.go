...
func main() {
	...
	for mem[rt-1][ct-1] == -1 {
		...
		for p.x+1 < h && string(sMatrix[p.x+1][p.y]) != "#" {
			p.x += 1
			if mem[p.x][p.y] != -1 {
				break
			}
			mem[p.x][p.y] = mem[s.x][s.y] + 1
		...
		for p.x-1 >= 0 && string(sMatrix[p.x-1][p.y]) != "#" {
			p.x -= 1
			if mem[p.x][p.y] != -1 {
				break
			}
		...
		for p.y+1 < w && string(sMatrix[p.x][p.y+1]) != "#" {
			p.y += 1
			if mem[p.x][p.y] != -1 {
				break
			}
		...
		for p.y-1 >= 0 && string(sMatrix[p.x][p.y-1]) != "#" {
			p.y -= 1
			if mem[p.x][p.y] != -1 {
				break
			}
