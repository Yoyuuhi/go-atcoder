...
var maxLength = make([]int, 1)
var endNode = make([]int, 1)
...
func main() {
	...
	for _, edge := range edges[0] {
		findMaxLength(edges, 0, edge.to, 1)
	}

	for _, edge := range edges[endNode[0]] {
		findMaxLength(edges, endNode[0], edge.to, 1)
	}

	out(maxLength[0] + 1)
}

func findMaxLength(edges [][]edge, former, now, length int) {
	if len(edges[now]) == 1 {
	...
	for _, edge := range edges[now] {
		if edge.to != former {
			findMaxLength(edges, now, edge.to, length+1)
		}
	}
}
