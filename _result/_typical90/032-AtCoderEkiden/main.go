...
func main() {
	...
	if m != 0 {
		for i := 0; i < m; i++ {
			...
			badRel[x] = append(badRel[x], y)
			badRel[y] = append(badRel[y], x)
		}
	}

	pRest := make(map[int]int, n)
	for i := 0; i < n; i++ {
		pRest[i] = 0
	}
	...
	for k, _ := range pRest {
		newPRest := make(map[int]int, len(pRest))
		for key, val := range pRest {
			newPRest[key] = val
		}
		delete(newPRest, k)

		simulate(0, -1, k, 0, &result, newPRest, as, badRel)
	}
...
func simulate(sec, pFormer, pNow int, time int, result *int, pRest map[int]int, as [][]int, badRel map[int][]int) {
	for _, p := range badRel[pFormer] {
		if p == pNow {
			return
		}
	}
	...
	for k, _ := range pRest {
		...
		simulate(sec+1, pNow, k, time, result, newPRest, as, badRel)
	}
}
