...
func main() {
	...
	sumNewAs := make([]int, 2*n)
	sumNewAs[0] = newAs[0]
	for i := 1; i < 2*n; i++ {
		sumNewAs[i] += newAs[i] + sumNewAs[i-1]
	}

	for i := 0; i < 2*n; i++ {
		rightIndex := bs(0, 2*n-1, func(c int) bool {
			return sumNewAs[c]-sumNewAs[i] < total/10
		})

		if rightIndex < 2*n-1 && sumNewAs[rightIndex+1]-sumNewAs[i] == total/10 && sumNewAs[rightIndex+1]-sumNewAs[i] != 0 {
			out("Yes")
			return
		}
	}
