...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		edgesConvert[t] = append(edgesConvert[t], edge{to: s})
	}

	// node: label
	nodeLabel := make([]int, n)
	label := 1

	// label: node
	labelNode := make(map[int]int, n)

	for node, _ := range nodeLabel {
		if nodeLabel[node] == 0 {
			lableNodes(node, &label, edges, nodeLabel, labelNode)
		}
	}
	...
	for i := n; i > 0; i-- {
		count := 0
		if labelNode[i] != -1 {
			countGroup(labelNode[i], i, edgesConvert, &count, nodeLabel, labelNode)
		}
		ans += count * (count - 1) / 2
	}
	out(ans)
...
func lableNodes(start int, label *int, edges [][]edge, nodeLabel []int, labelNode map[int]int) {
	nodeLabel[start] = -1
	for _, nextEdge := range edges[start] {
		if nodeLabel[nextEdge.to] > 0 || nodeLabel[nextEdge.to] == -1 {
			continue
		}
		lableNodes(nextEdge.to, label, edges, nodeLabel, labelNode)
	}
	if nodeLabel[start] == -1 {
...
func countGroup(start int, label int, edgesConvert [][]edge, count *int, nodeLabel []int, labelNode map[int]int) {
	*count += 1
	labelNode[label] = -1

	for _, nextEdge := range edgesConvert[start] {
		if labelNode[nodeLabel[nextEdge.to]] == -1 {
			continue
		}
		countGroup(nextEdge.to, nodeLabel[nextEdge.to], edgesConvert, count, nodeLabel, labelNode)
	}
}
