...
func main() {
	...
	groupPoints := make(map[int][]pGroup)
	...
	for i := 0; i < q; i++ {
		...
		case 1:
			ri, ci := ni2()
			paintAndConnect(&groupCount, mem, groupPoints, ri, ci)
		case 2:
			rai, cai, rbi, cbi := ni4()
			if mem[rai][cai] != 0 && mem[rai][cai] == mem[rbi][cbi] {
				out("Yes")
			} else {
				out("No")
			}
...
func paintAndConnect(groupCount *int, mem [][]int, groupPoints map[int][]pGroup, ri, ci int) {
	besideGroup := 0
	switch true {
	case mem[ri-1][ci] > 0:
		besideGroup = mem[ri-1][ci]
	case mem[ri+1][ci] > 0:
		besideGroup = mem[ri+1][ci]
	case mem[ri][ci-1] > 0:
		besideGroup = mem[ri][ci-1]
	case mem[ri][ci+1] > 0:
		besideGroup = mem[ri][ci+1]
	default:
		mem[ri][ci] = *groupCount
		groupPoints[*groupCount] = append(groupPoints[*groupCount], pGroup{
			i: ri,
			j: ci,
		})
		*groupCount += 1
	...
	mem[ri][ci] = besideGroup
	groupPoints[besideGroup] = append(groupPoints[besideGroup], pGroup{
		i: ri,
		j: ci,
	})

	var besideDiffGroups []int
	if mem[ri-1][ci] > 0 && mem[ri-1][ci] != besideGroup {
		besideDiffGroups = append(besideDiffGroups, mem[ri-1][ci])
	}
	if mem[ri+1][ci] > 0 && mem[ri+1][ci] != besideGroup {
		besideDiffGroups = append(besideDiffGroups, mem[ri+1][ci])
	}
	if mem[ri][ci-1] > 0 && mem[ri][ci-1] != besideGroup {
		besideDiffGroups = append(besideDiffGroups, mem[ri][ci-1])
	}
	if mem[ri][ci+1] > 0 && mem[ri][ci+1] != besideGroup {
		besideDiffGroups = append(besideDiffGroups, mem[ri][ci+1])
	}

	for _, group := range besideDiffGroups {
		for _, pGroup := range groupPoints[group] {
			mem[pGroup.i][pGroup.j] = besideGroup
		}
		groupPoints[besideGroup] = append(groupPoints[besideGroup], groupPoints[group]...)
		groupPoints[group] = make([]pGroup, 0)
	}
}
