...
func main() {
	...
	colorNodes := make(map[int][]int, 2)
	nodeColor := make([]int, n)
	paintColor(0, 0, colorNodes, nodeColor, edges)

	if len(colorNodes[1]) >= n/2 {
		str := fmt.Sprint(colorNodes[1][0] + 1)
		for i := 1; i < n/2; i++ {
			str += " " + fmt.Sprint(colorNodes[1][i]+1)
		}
		out(str)
	} else {
		str := fmt.Sprint(colorNodes[2][0] + 1)
		for i := 1; i < n/2; i++ {
			str += " " + fmt.Sprint(colorNodes[2][i]+1)
		}
		out(str)
	}
}

func paintColor(now, from int, colorNodes map[int][]int, nodeColor []int, edges [][]edge) {
	if nodeColor[now] == 0 {
		switch nodeColor[from] {
		case 1:
			nodeColor[now] = 2
			colorNodes[2] = append(colorNodes[2], now)
		case 2:
			nodeColor[now] = 1
			colorNodes[1] = append(colorNodes[1], now)
		default:
			nodeColor[now] = 1
			colorNodes[1] = append(colorNodes[1], now)
		}
	...
	for _, nextEdge := range edges[now] {
		if nodeColor[nextEdge.to] > 0 {
			continue
		}
		paintColor(nextEdge.to, now, colorNodes, nodeColor, edges)
	}
}
