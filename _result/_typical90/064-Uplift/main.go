...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if left != -1 {
			templeft := deltaAn[left] - v
			deltaAbs := abs(templeft) - abs(deltaAn[left])
			ans += deltaAbs
			deltaAn[left] = templeft
		}
		if right != n-1 {
			tempRight := deltaAn[right] + v
			deltaAbs := abs(tempRight) - abs(deltaAn[right])
			ans += deltaAbs
			deltaAn[right] = tempRight
		}
		out(ans)
	}
}
