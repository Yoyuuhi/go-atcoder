...
func main() {
	...
	applicable, ans := simulate(h, w, aMatrix, bMatrix)
	if !applicable {
		out("No")
	} else {
		out("Yes")
...
func simulate(h, w int, aMatrix, bMatrix [][]int) (bool, int) {
	memRow := make([][]int, h-1)
	ans := 0
	for row := 0; row < h-1; row++ {
		memRow[row] = make([]int, w-1)

		add := 0
		if row > 0 {
			add = memRow[row-1][0]
		}
		memRow[row][0] = bMatrix[row][0] - aMatrix[row][0] - add
		ans += abs(memRow[row][0])
		for i := 1; i < w-1; i++ {
			if row > 0 {
				add = memRow[row-1][i-1] + memRow[row-1][i]
			}
			memRow[row][i] = bMatrix[row][i] - aMatrix[row][i] - memRow[row][i-1] - add
			ans += abs(memRow[row][i])
		}
		if row > 0 {
			add = memRow[row-1][w-2]
		}
		if memRow[row][w-2]+aMatrix[row][w-1]+add != bMatrix[row][w-1] {
			return false, 0
		}
	}

	if memRow[h-2][0]+aMatrix[h-1][0] != bMatrix[h-1][0] {
		return false, 0
	}
	for i := 1; i < w-1; i++ {
		if memRow[h-2][i-1]+memRow[h-2][i]+aMatrix[h-1][i] != bMatrix[h-1][i] {
			return false, 0
		}
	}
	if memRow[h-2][w-2]+aMatrix[h-1][w-1] != bMatrix[h-1][w-1] {
		return false, 0
	}
	return true, ans
}
