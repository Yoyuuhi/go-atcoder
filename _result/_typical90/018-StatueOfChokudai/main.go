...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		var ang float64 = float64(ei) / float64(t) * 360 * math.Pi / 180.0

		xi := 0.0
		yi := -1.0 * math.Sin(ang) * float64(l) / 2.0
		zi := float64(l)/2.0 + math.Cos(ang-math.Pi)*float64(l)/2.0

		length := math.Sqrt((float64(x)-xi)*(float64(x)-xi) + (float64(y)-yi)*(float64(y)-yi))
		height := zi

		out(FloatFormatWithMaxPrecision(math.Atan(height/length)/math.Pi*180.0, 12, true))
	}
}
