...
func main() {
	...
	wfs(sx, point{rs, cs}, point{rt, ct}, h, w)

	ans := inf
	for i := 0; i < 4; i++ {
		if mem[rt][ct][i] != 0 {
			ans = min(ans, mem[rt][ct][i])
		}
	}
	out(ans - 1)
}

func wfs(sX []string, start, to point, h, w int) {
	l := list.New()
	l.PushBack(start)
	lE := l.Front()
	...
	for lE != nil {
		...
		for vec, v := range vectorsA {
			minV := minVal(vec, now.x, now.y)
			toX := now.x + v.x
			toY := now.y + v.y

			for toX < h && toX >= 0 && toY < w && toY >= 0 && string(sX[toX][toY]) != "#" {
				minV1 := minVal(vec, toX, toY)

				if minV < minV1 {
					mem[toX][toY][vec] = minV
...
func minVal(vec int, x, y int) int {
	minV1 := inf
	for vec1, v1 := range mem[x][y] {
		if v1 == -1 {
			return 1
		}
