...
func main() {
	...
	n := ns()
	sc.Scan()
	k, _ := strconv.ParseUint(sc.Text(), 10, 64)
	...
	var i uint64
	for i = 0; i < k; i++ {
		tens = convertToTens(n, 8)
		result = convertToNums(tens, 9)
		n = result
	}
...
func convertToTens(str string, formerNum uint64) uint64 {
	len := len(str)

	var forNum uint64 = 1
	var ans uint64 = 0
	for i := len - 1; i >= 0; i-- {
		numI, _ := strconv.ParseUint(string(str[i]), 10, 64)
...
func convertToNums(tens, nextNum uint64) string {
	main := tens
	var res uint64 = 0
	var numArr []uint64
	...
	for main != 0 {
		...
		numArr = append(numArr, res)
	}

	var tenNum uint64 = 1
	var result uint64 = 0
	for i := 0; i < len(numArr); i++ {
		result += tenNum * numArr[i]
		tenNum *= 10
	}
