...
func main() {
	...
	defer flush()
	sc.Scan()
	l, _ := strconv.ParseUint(sc.Text(), 10, 64)
	sc.Scan()
	r, _ := strconv.ParseUint(sc.Text(), 10, 64)

	strL := strconv.FormatUint(l, 10)
	strR := strconv.FormatUint(r, 10)

	if len(strL) == len(strR) {
		var temp1 uint64
		var temp2 uint64
		if (l%2 == 0 && r%2 == 0) || (l%2 != 0 && r%2 != 0) {
			temp1 = (l + r) / 2 % mod1000000007
			temp2 = (r - l + 1) % mod1000000007
		} else {
			temp1 = (l + r) % mod1000000007
			temp2 = (r - l + 1) / 2 % mod1000000007
		}
		out(uint64(len(strL)) * temp1 * temp2 % mod1000000007)
		return
	}
	...
	ninesBeforeI, _ := strconv.ParseUint(ninesBeforeS, 10, 64)
	var temp1 uint64
	var temp2 uint64
	if (l%2 == 0 && ninesBeforeI%2 == 0) || (l%2 != 0 && ninesBeforeI%2 != 0) {
		temp1 = (l + ninesBeforeI) / 2 % mod1000000007
		temp2 = (ninesBeforeI - l + 1) % mod1000000007
	} else {
		temp1 = (l + ninesBeforeI) % mod1000000007
		temp2 = (ninesBeforeI - l + 1) / 2 % mod1000000007
	}
	ans += uint64(len(strL)) * temp1 * temp2 % mod1000000007
	ans %= mod1000000007

	for i := len(strL) + 1; i < len(strR); i++ {
		ninesMidS := ""
		oneMidS := ""
		for j := 0; j < i; j++ {
			ninesMidS += "9"
		...
		ninesMidI, _ := strconv.ParseUint(ninesMidS, 10, 64)
		oneMidI, _ := strconv.ParseUint(oneMidS, 10, 54)
		var temp1 uint64
		var temp2 uint64
		if (oneMidI%2 == 0 && ninesMidI%2 == 0) || (oneMidI%2 != 0 && ninesMidI%2 != 0) {
			temp1 = (oneMidI + ninesMidI) / 2 % mod1000000007
			temp2 = (ninesMidI - oneMidI + 1) % mod1000000007
		} else {
			temp1 = (oneMidI + ninesMidI) % mod1000000007
			temp2 = (ninesMidI - oneMidI + 1) / 2 % mod1000000007
		}
		ans += uint64(i) * temp1 * temp2 % mod1000000007
		ans %= mod1000000007
	}
	...
	oneAfterI, _ := strconv.ParseUint(oneAfterS, 10, 64)
	if (oneAfterI%2 == 0 && r%2 == 0) || (oneAfterI%2 != 0 && r%2 != 0) {
		temp1 = (oneAfterI + r) / 2 % mod1000000007
		temp2 = (r - oneAfterI + 1) % mod1000000007
	} else {
		temp1 = (oneAfterI + r) % mod1000000007
		temp2 = (r - oneAfterI + 1) / 2 % mod1000000007
	}
	ans += uint64(len(strR)) * temp1 * temp2 % mod1000000007
	ans %= mod1000000007
