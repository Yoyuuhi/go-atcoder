...
func main() {
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			simulate(cMatrix, walked, i, j, i, j, 0, &maxLoop)
		}
	}
	out(maxLoop)
}

func simulate(cMatrix []string, walked [][]int, xStart, yStart, xNow, yNow, nowCount int, maxLoop *int) {
	if xStart == xNow && yStart == yNow && nowCount >= 4 {
		if *maxLoop < nowCount {
			*maxLoop = nowCount
		}
		return
	}
	if string(cMatrix[xNow][yNow]) == "#" || walked[xNow][yNow] != 0 {
		return
	}
	nowCount += 1

	copyWalked := make([][]int, h)
	for i := 0; i < h; i++ {
		copyWalked[i] = make([]int, w)
		copy(copyWalked[i], walked[i])
	}
	copyWalked[xNow][yNow] = 1

	if xNow < h-1 {
		simulate(cMatrix, copyWalked, xStart, yStart, xNow+1, yNow, nowCount, maxLoop)
	}
	if xNow > 0 {
		simulate(cMatrix, copyWalked, xStart, yStart, xNow-1, yNow, nowCount, maxLoop)
	}
	if yNow < w-1 {
		simulate(cMatrix, copyWalked, xStart, yStart, xNow, yNow+1, nowCount, maxLoop)
	}
	if yNow > 0 {
		simulate(cMatrix, copyWalked, xStart, yStart, xNow, yNow-1, nowCount, maxLoop)
	}
