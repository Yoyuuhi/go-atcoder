...
func main() {
	...
	if len(strL) == len(strR) {
		...
		if (l%2 == 0 && r%2 == 0) || (l%2 != 0 && r%2 != 0) {
			temp1 = mdiv(madd(l, r), 2)
			temp2 = madd(madd(r, -l), 1)
		} else {
			temp1 = madd(l, r)
			temp2 = mdiv(madd(madd(r, -l), 1), 2)
		}
		ans := mmul((len(strL)), mmul(temp1, temp2))
		out(ans)
		return
	}
	...
	ninesBeforeI := atoi(ninesBeforeS)
	var temp1 int
	var temp2 int
	if (l%2 == 0 && ninesBeforeI%2 == 0) || (l%2 != 0 && ninesBeforeI%2 != 0) {
		temp1 = mdiv(madd(l, ninesBeforeI), 2)
		temp2 = madd(madd(ninesBeforeI, -l), 1)
	} else {
		temp1 = madd(l, ninesBeforeI)
		temp2 = mdiv(madd(madd(ninesBeforeI, -l), 1), 2)
	}
	ans = madd(ans, mmul((len(strL)), mmul(temp1, temp2)))
	...
	for i := len(strL) + 1; i < len(strR); i++ {
		...
		ninesMidI := atoi(ninesMidS)
		oneMidI := atoi(oneMidS)
		var temp1 int
		var temp2 int
		if (oneMidI%2 == 0 && ninesMidI%2 == 0) || (oneMidI%2 != 0 && ninesMidI%2 != 0) {
			temp1 = mdiv(madd(oneMidI, ninesMidI), 2)
			temp2 = madd(madd(ninesMidI, -oneMidI), 1)
		} else {
			temp1 = madd(oneMidI, ninesMidI)
			temp2 = mdiv(madd(madd(ninesMidI, -oneMidI), 1), 2)
		}
		ans = madd(ans, mmul((i), mmul(temp1, temp2)))
	}
	...
	oneAfterI := atoi(oneAfterS)
	if (oneAfterI%2 == 0 && r%2 == 0) || (oneAfterI%2 != 0 && r%2 != 0) {
		temp1 = mdiv(madd(oneAfterI, r), 2)
		temp2 = madd(madd(r, -oneAfterI), 1)
	} else {
		temp1 = madd(oneAfterI, r)
		temp2 = mdiv(madd(madd(r, -oneAfterI), 1), 2)
	}
	ans = madd(ans, mmul((len(strR)), mmul(temp1, temp2)))
