...
func main() {
	...
	ans := 0
	for bits := 0; bits < (1 << uint64(h)); bits++ {
		tempAns := simulate(h, w, bits, pMat)
		if ans < tempAns {
			ans = tempAns
		}
	}
...
func simulate(h, w, bits int, pMat [][]int) int {
	bitMap := make(map[int]int, h)
	for i := 0; i < h; i++ {
...
colLoop:
	for j := 0; j < w; j++ {
		...
		if maxCol < mem[firstNum] {
			maxCol = mem[firstNum]
		}
	}
	return len(bitMap) * maxCol
}
