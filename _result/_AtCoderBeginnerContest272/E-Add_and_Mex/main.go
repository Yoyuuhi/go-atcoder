...
func main() {
	...
	effectiveTurnVal := map[int]map[int]bool{}
	for i := 0; i < n; i++ {
		l := 0
		if as[i] < 0 {
			switch true {
		...
		for j := l; j <= r; j++ {
			if _, e := effectiveTurnVal[j]; !e {
				effectiveTurnVal[j] = map[int]bool{}
			}
			effectiveTurnVal[j][as[i]+(i+1)*j] = true
		}
	}

	for i := 1; i <= m; i++ {
		ans := 0
		for effectiveTurnVal[i][ans] {
			ans++
		}
		out(ans)
	}
