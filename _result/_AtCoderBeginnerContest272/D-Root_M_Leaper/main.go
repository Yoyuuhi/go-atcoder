...
func main() {
	...
	for i := 0; i*i <= m; i++ {
		for j := 0; i*i+j*j <= m; j++ {
			if i*i+j*j == m {
				steps = append(steps, point{x: i, y: j})
				steps = append(steps, point{x: -i, y: j})
				steps = append(steps, point{x: i, y: -j})
				steps = append(steps, point{x: -i, y: -j})
			}
	...
	for e != nil {
		...
		for _, step := range steps {
			if x+step.x < n && x+step.x >= 0 && y+step.y < n && y+step.y >= 0 {
				x1 := x + step.x
				y1 := y + step.y
				if ansX[x1][y1] == -1 {
					ansX[x1][y1] = stepBefore + 1
					pList.PushBack(point{x: x1, y: y1})
				}
			}
		}

		e = e.Next()
		pList.Remove(pList.Front())
	}
