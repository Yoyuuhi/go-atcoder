...
func main() {
	...
	outwoln(convNum(ans[0]))
	for i := 1; i < len(ans); i++ {
		outwoln(" ", convNum(ans[i]))
	}
	out()
}

func convNum(a []int) int {
	ans := 0
	for i, a1 := range a {
		if a1 == 1 {
			ans += 1 << i
...
func calc(mem [][]int, nth int, ans [][]int) {
	bit := mem[0][nth]
