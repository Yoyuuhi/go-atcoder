...
func main() {
	...
	edgesA := make([][]edge, n)
	for i, st := range sts {
		if abs(dist[st[0]]-dist[st[1]]) == st[2] {
			s := st[0]
			t := st[1]
			c := st[2]
			idx := i + 1
			edgesA[s] = append(edgesA[s], edge{to: t, cost: c, idx: idx})
			edgesA[t] = append(edgesA[t], edge{to: s, cost: c, idx: idx})
		}
	}
