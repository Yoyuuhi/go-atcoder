...
func graphDfs(edges [][]edge, now int) {
	if !poss {
		return
	}
	passed[now] = true
	...
	for _, edge := range edges[now] {
		if !passed[edge.to] {
			mem[edge.to] = mem[now] + edge.cost
			vmin = min(vmin, mem[edge.to])
			vmax = max(vmax, mem[edge.to])
			graphDfs(edges, edge.to)
		} else if mem[edge.to] != mem[now]+edge.cost {
			poss = false
			return
		}
	}
