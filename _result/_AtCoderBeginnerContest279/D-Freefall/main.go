...
func main() {
	...
	a, b := float64(ni()), float64(ni())
	g := float64(1)
	ans := a / math.Sqrt(g)

	x := bs3fl(0, a/b+2, 500, func(x float64) float64 {
		return a/math.Sqrt(g+x) + b*x
	})
	x = math.Round(x)
	ans = minf(ans, a/math.Sqrt(g+x)+b*x)
	if x > 0 {
		ans = minf(ans, a/math.Sqrt(g+x-1)+b*(x-1))
	}
	if x > 1 {
		ans = minf(ans, a/math.Sqrt(g+x-2)+b*(x-2))
	}
	ans = minf(ans, a/math.Sqrt(g+x+1)+b*(x+1))
	ans = minf(ans, a/math.Sqrt(g+x+2)+b*(x+2))
	fmt.Printf("%.10f\n", ans)
