...
func main() {
	...
	dpLeft := make([]int, n)
	memSumLeft := make([]int, n)
	memSumLeft[0] = as[0]
	for i := 1; i < n; i++ {
		memSumLeft[i] = memSumLeft[i-1] + as[i]
	}
	for i := 0; i < n; i++ {
		dpLeft[i] = min(l*(i+1), memSumLeft[i])
		if i > 0 {
			dpLeft[i] = min(l*(i+1), dpLeft[i-1]+as[i])
		}
	}

	dpRight := make([]int, n)
	memSumRight := make([]int, n)
	memSumRight[n-1] = as[n-1]
	for i := n - 2; i >= 0; i-- {
		memSumRight[i] = memSumRight[i+1] + as[i]
	}
	for i := n - 1; i >= 0; i-- {
		dpRight[i] = min(r*(n-i), memSumRight[i])
		if i < n-1 {
			dpRight[i] = min(r*(n-i), dpRight[i+1]+as[i])
		}
	}

	ans := inf
	for i := 0; i < n-1; i++ {
		ans = min(ans, dpLeft[i]+dpRight[i+1])
	}
	ans = min(ans, dpLeft[n-1])
	ans = min(ans, dpRight[0])
	out(ans)
}
