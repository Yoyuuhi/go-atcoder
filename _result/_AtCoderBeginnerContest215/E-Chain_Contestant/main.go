...
func main() {
	...
	for i := 0; i < n; i++ {
		dp[i+1] = map[int]map[int]int{}
		l := level(string(s[i]))
		for k1, ma := range dp[i] {
			for k2, v := range ma {
				if _, e := dp[i+1][k1]; !e {
					dp[i+1][k1] = map[int]int{}
				}
				dp[i+1][k1][k2] = madd(v, dp[i+1][k1][k2])

				if !hasbit(k1, l) || 1<<l == k2 {
					if _, e := dp[i+1][k1|(1<<l)]; !e {
						dp[i+1][k1|(1<<l)] = map[int]int{}
					}
					dp[i+1][k1|(1<<l)][1<<l] = madd(v, dp[i+1][k1|(1<<l)][1<<l])
				}
			}
		}
	}
	ans := 0
	for k1, ma := range dp[n] {
		if k1 == 0 {
			continue
		}
		for _, v := range ma {
