...
func main() {
	...
	for k > 0 {
		level := calcLevel(mem, k)
		dequeBack.PushBack(level)
		k -= mem[level]
	}
	frontE := dequeBack.Back()
	dequeFront := list.New()

	for frontE != nil && frontE.Value != 0 {
		dequeFront.PushFront(frontE.Value.(int))
		frontE = frontE.Prev()
	}

	frontE = dequeFront.Front()
	levels := frontE.Value.(int)
	for i := levels; i > 0; i-- {
		if frontE != nil && i == frontE.Value.(int) {
			outwoln(2)
			frontE = frontE.Next()
			continue
		}
...
func calcLevel(mem map[int]int, num int) int {
	return bs(1, len(mem), func(c int) bool {
		return mem[c] <= num
	})
}
