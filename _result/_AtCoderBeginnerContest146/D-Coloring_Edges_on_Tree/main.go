...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, i: i})
		edges[t] = append(edges[t], edge{to: s, i: i})
	}
	ans = make([]int, n-1)
	graphDfs(edges, 0, 0, map[int]bool{})
	count := map[int]bool{}
	for _, a := range ans {
		count[a] = true
	}
...
func graphDfs(edges [][]edge, now int, prevC int, passed map[int]bool) {
	passed[now] = true
	c := 1
	...
	for _, edge := range edges[now] {
		if !passed[edge.to] {
			...
			ans[edge.i] = c
			graphDfs(edges, edge.to, c, passed)
			c++
		}
	}
}
