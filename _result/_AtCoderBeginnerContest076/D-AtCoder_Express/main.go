...
func main() {
	...
	for i := range mem {
		...
		mem[i] = min(vs[i-1], vs[i])
	}
	for i := 1; i <= n; i++ {
		mem[i] = min(mem[i], mem[i-1]+ts[i-1])
		mem[i-1] = min(mem[i-1], mem[i]+ts[i-1])
	}
	for i := n; i >= 1; i-- {
		mem[i] = min(mem[i], mem[i-1]+ts[i-1])
		mem[i-1] = min(mem[i-1], mem[i]+ts[i-1])
	...
	var ans float64
	for i := 0; i < n; i++ {
		ans += calc(float64(mem[i]), float64(mem[i+1]), float64(ts[i]), float64(vs[i]))
	}
	out(ans)
}

func calc(l, r, t, v float64) float64 {
	vm := minf(v, (t+l+r)/2.0)
	t1 := vm - l
	t2 := vm - r
	t3 := t - t1 - t2
	return t1*(vm+l)/2.0 + t2*(vm+r)/2.0 + t3*vm
}
