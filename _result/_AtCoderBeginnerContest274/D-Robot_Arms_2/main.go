...
func main() {
	...
	possX := simulateX(tarX, asX, minX, maxX)
	possY := simulateX(tarY, asY, minY, maxY)
	if possX && possY {
		out("Yes")
	} else {
		out("No")
...
func simulateX(tar int, asXY []int, minXY, maxXY int) bool {
	mem := make([]map[int]bool, len(asXY)+1)
	mem[0] = map[int]bool{}
	mem[0][0] = true

	for i, a := range asXY {
		mem[i+1] = map[int]bool{}
		for xy, _ := range mem[i] {
			if xy-a > minXY {
				mem[i+1][xy-a] = true
			}
			if xy+a < maxXY {
	...
	mem[len(asXY)][minXY] = true
	mem[len(asXY)][maxXY] = true
	return mem[len(asXY)][tar]
}
