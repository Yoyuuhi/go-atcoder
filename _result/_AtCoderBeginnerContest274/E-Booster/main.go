...
func main() {
	...
	distO := make([]float64, n+m)
	for i := 0; i < n+m; i++ {
		distO[i] = math.Sqrt(float64(ps[i][0]*ps[i][0] + ps[i][1]*ps[i][1]))
	}

	dist := make([][]float64, n+m)
	for i := 0; i < m+n; i++ {
		dist[i] = make([]float64, n+m)
	}
	for i := 0; i < m+n-1; i++ {
		for j := i + 1; j < m+n; j++ {
			p1 := ps[i]
			...
			delX := p1[0] - p2[0]
			delY := p1[1] - p2[1]
			dist[i][j] = math.Sqrt(float64(delX*delX + delY*delY))
			dist[j][i] = dist[i][j]
		}
	}
	...
	for j := 1; j < 1<<(n+m); j++ {
		for i := 0; i < n+m; i++ {
			...
			s := float64(speed(j, n, m))
			for to := 0; to < n+m; to++ {
				if i == to || j>>to&1 != 0 {
					continue
				}
				toBit := 1<<to | j
				toVal := dp[i][j] + dist[i][to]/s
				if dp[to][toBit] == 0 || dp[to][toBit] > toVal {
					dp[to][toBit] = toVal
				}
			}
		}
	...
	for i, d := range dp {
		for k, v := range d {
			...
			if k&(1<<n-1) == 1<<n-1 {
				s := float64(speed(k, n, m))
				valFinal := v + distO[i]/s
				if valFinal < ans {
					ans = valFinal
				}
			}
