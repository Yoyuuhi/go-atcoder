	...
	for i := h; i > 0; i-- {
		for j := w; j > 0; j-- {
			...
			if (i+j)%2 == 0 {
				dp[i][j] = -inf
				if i+1 <= h {
					dp[i][j] = max(dp[i][j], dp[i+1][j]+calc(i+1, j, aX))
				}
				if j+1 <= w {
					dp[i][j] = max(dp[i][j], dp[i][j+1]+calc(i, j+1, aX))
				}
			} else {
				dp[i][j] = inf
				if i+1 <= h {
					dp[i][j] = min(dp[i][j], dp[i+1][j]-calc(i+1, j, aX))
				}
				if j+1 <= w {
					dp[i][j] = min(dp[i][j], dp[i][j+1]-calc(i, j+1, aX))
				}
			}
		}
	}
...
func calc(i, j int, aX []string) int {
	switch string(aX[i-1][j-1]) {
	case "+":
		return 1
	default:
		return -1
