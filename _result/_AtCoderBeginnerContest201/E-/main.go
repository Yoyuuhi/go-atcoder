...
func main() {
	...
	mem = make([]int, n)
	graphDfs(edges, 0, map[int]bool{}, 0)

	memCount := make([]int, 60)
	for _, num := range mem {
		for i := 0; i < 60; i++ {
			if hasbit(num, i) {
				memCount[i] += 1
	...
	ans := 0
	for i := 0; i < 60; i++ {
		ans = madd(ans, mmul(memCount[i], mmul(n-memCount[i], mpow(2, i, mod))))
	}
	out(ans)
}

func graphDfs(edges [][]edge, now int, passed map[int]bool, val int) {
	passed[now] = true
	mem[now] = val

	for _, edge := range edges[now] {
		if !passed[edge.to] {
			graphDfs(edges, edge.to, passed, val^edge.cost)
		}
	}
}
