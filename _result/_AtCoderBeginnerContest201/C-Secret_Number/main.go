...
func main() {
	...
	s := ns()
	memMust := map[int]bool{}
	memPoss := map[int]bool{}
	...
	keys := make([]int, len(memMust)+len(memPoss))
	i := 0
	for k, _ := range memMust {
		keys[i] = k
		i++
	}
	for k, _ := range memPoss {
		keys[i] = k
		i++
	}
	...
	memAns := map[int]bool{}
	simulate(memMust, memPoss, keys, 0, map[int]int{}, 0, memAns)
	out(len(memAns))
}

func simulate(memMust, memPoss map[int]bool, keys []int, now int, passed map[int]int, ans int, memAns map[int]bool) {
	if now == 4 {
		for must, _ := range memMust {
			if passed[must] == 0 {
				return
			}
		}
	...
	for _, key := range keys {
		passed[key] += 1
		simulate(memMust, memPoss, keys, now+1, passed, ans*10+key, memAns)
		passed[key] -= 1
	}
}
