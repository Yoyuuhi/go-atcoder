	...
	for i := m; i < n; i++ {
		if as[i-m] < upper {
			...
			if as[i] < upper {
				...
				if memUpper[upper] >= rbm.Lookup(upper) {
					memUpper[upper] = 0
					upper, _ = rbm.UpperBound(upper)
				}
				memUpper[upper] += 1
				sum += upper
			}
			outwoln(" ", sum)
		} else if as[i-m] == upper {
			rbm.Decrement(as[i-m])
			rbm.Increment(as[i])
			sum -= upper
			if as[i] < upper {
				if memUpper[upper] > 1 {
					...
					memUpper[upper] = 0
					upper, _ = rbm.LowerBound(upper)
					memUpper[upper] = rbm.Lookup(upper)
				}
				sum += as[i]
			} else if as[i] == upper {
				sum += upper
