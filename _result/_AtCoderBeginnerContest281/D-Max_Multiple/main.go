...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < k; j++ {
			for key := range dp[i][j] {
				if k-j <= n-i-1 {
					dp[i+1][j][key] = max(dp[i+1][j][key], dp[i][j][key])
				}
				if k-j-1 <= n-i-1 {
					if j+1 == k {
						if (key+as[i])%d == 0 {
							ans = max(ans, dp[i][j][key]+as[i])
						}
					} else {
						dp[i+1][j+1][(key+as[i])%d] = max(dp[i+1][j+1][(key+as[i])%d], dp[i][j][key]+as[i])
					}
				}
			}
		}
