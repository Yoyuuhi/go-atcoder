	...
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			for k := 0; k < 4; k++ {
				...
				x1, e := calc(xm, xp, i, mem)
				if !e {
					continue
				}
				x2, e := calc(xm, xp, j, mem)
				if !e {
					continue
				}
				x3, e := calc(xm, xp, k, mem)
				if !e {
					continue
				}

				ma = max(ma, float64(x1+x2+x3)/float64(x1*x2*x3))
				mi = min(mi, float64(x1+x2+x3)/float64(x1*x2*x3))
			}
		}
	}
	out(mi)
...
func calc(xm, xp []int, i int, mem map[int]int) (int, bool) {
	var ans int
	switch i {
	case 0:
		if len(xm) == 0 || mem[i] > mem[1] {
			return 0, false
		}
		if mem[i] >= len(xm) {
			return 0, false
