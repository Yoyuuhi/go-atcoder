...
func main() {
	...
	for _, e := range es {
		val1, exist1 := ansMap[edges[e-1].from]
		if !exist1 {
			continue
		}

		val2, exist2 := ansMap[edges[e-1].to]
		if !exist2 {
			ansMap[edges[e-1].to] = val1 + edges[e-1].cost
			continue
		}

		ansMap[edges[e-1].to] = min(val2, val1+edges[e-1].cost)
	}
