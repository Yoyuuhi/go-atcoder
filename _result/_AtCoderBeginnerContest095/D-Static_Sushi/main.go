...
func main() {
	...
	for i := 0; i < n; i++ {
		x, v := ni2()
		xvs = append(xvs, []int{x, v})
		xvs = append(xvs, []int{x - c, v})
	}
	sort.Slice(xvs, func(i, j int) bool {
		return xvs[i][0] < xvs[j][0]
	})
	// out(xvs)

	mem1 := make([]int, 2*n)
	mem2 := make([]int, 2*n)
	mem1[n], mem1[n-1] = xvs[n][1]-abs(xvs[n][0]), xvs[n-1][1]-abs(xvs[n-1][0])
	mem2[n], mem2[n-1] = xvs[n][1]-2*abs(xvs[n][0]), xvs[n-1][1]-2*abs(xvs[n-1][0])

	for i := n - 2; i >= 0; i-- {
		mem1[i] = mem1[i+1] + xvs[i][1] - abs(xvs[i][0]-xvs[i+1][0])
		mem2[i] = mem2[i+1] + xvs[i][1] - 2*abs(xvs[i][0]-xvs[i+1][0])
	}
	for i := n + 1; i < 2*n; i++ {
		mem1[i] = mem1[i-1] + xvs[i][1] - abs(xvs[i][0]-xvs[i-1][0])
		mem2[i] = mem2[i-1] + xvs[i][1] - 2*abs(xvs[i][0]-xvs[i-1][0])
	...
	ans := max(mem1[0], mem1[2*n-1])
	ans = max(ans, 0)
	for i := 1; i < n; i++ {
		j := n - 1 + i
		ans = max(ans, mem1[i]+mem2[j])
		ans = max(ans, mem2[i]+mem1[j])
	}
	// out(mem1, mem2)
	out(ans)
}
