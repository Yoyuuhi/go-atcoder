...
func main() {
	...
	n, m = ni2()
	mm = []int{-1, 2, 5, 5, 4, 5, 6, 3, 7, 6}
	as = nis(m)

	mem = map[int]int{}
	for _, a := range as {
		mem[mm[a]] = max(mem[mm[a]], a)
	}
	if mem[2] != 0 && mem[4] != 0 {
		mem[4] = -1
	}
...
func bigger(ma, mb map[int]int) bool {
	if ma[-1] == 1 {
		return false
	}
