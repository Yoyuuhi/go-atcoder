...
func main() {
	...
	for i := 0; i < 2*n; i++ {
		...
		switch c {
		case "R":
			mem[0] = append(mem[0], a)
		case "G":
			mem[1] = append(mem[1], a)
		case "B":
			mem[2] = append(mem[2], a)
		}
	}
	sorti(mem[0])
	sorti(mem[1])
	...
	if len(mem[0])%2 == 0 && len(mem[1])%2 == 0 && len(mem[2])%2 == 0 {
		out(0)
		return
	}
	var a, b, c int
	...
	ans := inf
	a1 := inf / 2
	for _, av := range mem[a] {
		j := bs(0, len(mem[b])-1, func(idx int) bool {
			return mem[b][idx] <= av
		})
		ans = min(ans, abs(av-mem[b][max(j, 0)]))
		if j+1 < len(mem[b]) {
			ans = min(ans, abs(av-mem[b][j+1]))
		}

		if len(mem[c]) > 0 {
			j1 := bs(0, len(mem[c])-1, func(idx int) bool {
				return mem[c][idx] <= av
			})
			a1 = min(a1, abs(av-mem[c][max(j1, 0)]))
			if j1+1 < len(mem[c]) {
				a1 = min(a1, abs(av-mem[c][j1+1]))
			}
		}
	}

	a2 := inf / 2
	for _, bv := range mem[b] {
		if len(mem[c]) > 0 {
			j1 := bs(0, len(mem[c])-1, func(idx int) bool {
				return mem[c][idx] <= bv
			})
			a2 = min(a2, abs(bv-mem[c][max(j1, 0)]))
			if j1+1 < len(mem[c]) {
				a2 = min(a2, abs(bv-mem[c][j1+1]))
			}
		}
	}
