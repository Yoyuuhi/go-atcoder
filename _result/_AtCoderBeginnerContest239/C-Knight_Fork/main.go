...
func main() {
	...
	x1, y1, x2, y2 := ni4()
	mem := make(map[point]bool, 8)
	mem[point{x: x1 + 1, y: y1 + 2}] = true
	mem[point{x: x1 - 1, y: y1 + 2}] = true
	mem[point{x: x1 + 1, y: y1 - 2}] = true
	mem[point{x: x1 - 1, y: y1 - 2}] = true
	mem[point{x: x1 + 2, y: y1 + 1}] = true
	mem[point{x: x1 - 2, y: y1 + 1}] = true
	mem[point{x: x1 + 2, y: y1 - 1}] = true
	mem[point{x: x1 - 2, y: y1 - 1}] = true

	switch true {
	case mem[point{x: x2 + 1, y: y2 + 2}], mem[point{x: x2 - 1, y: y2 + 2}], mem[point{x: x2 + 1, y: y2 - 2}], mem[point{x: x2 - 1, y: y2 - 2}], mem[point{x: x2 + 2, y: y2 + 1}], mem[point{x: x2 - 2, y: y2 + 1}], mem[point{x: x2 + 2, y: y2 - 1}], mem[point{x: x2 - 2, y: y2 - 1}]:
		out("Yes")
	default:
		out("No")
	}
