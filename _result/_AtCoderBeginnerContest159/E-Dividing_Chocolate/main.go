...
func main() {
	...
	for bit := 0; bit < 1<<(h-1); bit++ {
		ans = min(ans, simulate(bit, mem, h, w, k))
		// out(bit, simulate(bit, mem, h, w, k))
	}
	out(ans)
}

func simulate(bit int, mem [][]int, h, w, k int) int {
	ans := 0
	hs := make([]int, 0, 10)
	hs = append(hs, 0)
	for i := 0; i < h-1; i++ {
	...
	for r <= w {
		...
		for i := 1; i < len(hs); i++ {
			u := hs[i-1]
			d := hs[i]
			if mem[d][r]-mem[d][l]-mem[u][r]+mem[u][l] > k {
				return inf
			}
			if r+1 <= w {
				next = next && mem[d][r+1]-mem[d][l]-mem[u][r+1]+mem[u][l] <= k
			}
		}
		if next {
			r++
