...
func main() {
	...
	calc(maxTimes, 0, n, 0, make([]int, maxTimes), cs)
	for _, v := range ans {
		outwoln(v)
	}
	out()
...
func calc(maxTimes, times, n, sum int, tempAns []int, cs []int) {
	if times == maxTimes {
		ans = tempAns
		finished = true
	}
	...
	for i := 8; i >= 0; i-- {
		if sum+cs[i]+minC*(maxTimes-times-1) <= n && !finished {
			tempAns[times] = i + 1
			calc(maxTimes, times+1, n, sum+cs[i], tempAns, cs)
		}
	}
}
