...
func main() {
	...
	mem := make(map[int]map[string]int, n)

	for i := 0; i < n; i++ {
		if mem[ws[i]] == nil {
			mem[ws[i]] = make(map[string]int, 2)
		}
		mem[ws[i]][string(s[i])] += 1
	}

	keys := make([]int, len(mem))
	index := 0
	for k, _ := range mem {
		keys[index] = k
		index++
	...
	sorti(keys)
	childSum := make([]int, len(keys)+1)
	childSum[0] = 0
	for i := 1; i <= len(keys); i++ {
		childSum[i] = childSum[i-1] + mem[keys[i-1]]["0"]
	}
	adultSum := make([]int, len(keys)+1)
	adultSum[len(keys)] = 0
	adultSum[len(keys)-1] = mem[keys[len(keys)-1]]["1"]
	for i := len(keys) - 2; i >= 0; i-- {
		adultSum[i] = adultSum[i+1] + mem[keys[i]]["1"]
	}

	ans := 0
	for i := 0; i <= len(keys); i++ {
		ans = max(ans, childSum[i]+adultSum[i])
	}
	out(ans)
}
