...
func main() {
	...
	for i := 0; i < n-1; i++ {
		for j := i + 1; j < n; j++ {
			temp := abs(xs[i]-xs[j]) + abs(ys[i]-ys[j])
			edges[i] = append(edges[i], edge{to: j, cost: temp})
			edges[j] = append(edges[j], edge{to: i, cost: temp})

			if temp%ps[i] == 0 {
				minS = min(minS, temp/ps[i])
				maxS = max(maxS, temp/ps[i])
			} else {
				minS = min(minS, temp/ps[i]+1)
				maxS = max(maxS, temp/ps[i]+1)
			}
		}
	}

	ans := bs(maxS, 0, func(s int) bool {
		for i := 0; i < n; i++ {
			mem := make(map[int]int, n)
			graphDfs(edges, s, i, mem, ps)
			if len(mem) == n {
				return true
			}
		}
...
func graphDfs(edges [][]edge, s, now int, passed map[int]int, ps []int) {
	passed[now] = 1
	...
	for _, edge := range edges[now] {
		...
		if edge.cost <= s*ps[now] {
			graphDfs(edges, s, edge.to, passed, ps)
		}
	}
}
