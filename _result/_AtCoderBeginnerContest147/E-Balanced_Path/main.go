...
func main() {
	...
	mem := make([][][12801]bool, h)
	for i := 0; i < h; i++ {
		mem[i] = make([][12801]bool, w)
	}
	mem[0][0][abs(ax[0][0]-bx[0][0])] = true

	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			for k, e := range mem[i][j] {
				if !e {
					continue
				}
				if i+1 < h {
					mem[i+1][j][abs(k+ax[i+1][j]-bx[i+1][j])] = true
					mem[i+1][j][abs(k+bx[i+1][j]-ax[i+1][j])] = true
				}
				if j+1 < w {
					mem[i][j+1][abs(k+ax[i][j+1]-bx[i][j+1])] = true
					mem[i][j+1][abs(k+bx[i][j+1]-ax[i][j+1])] = true
				}
			}
		}
	}

	for k, e := range mem[h-1][w-1] {
		if !e {
			continue
		}
		out(k)
