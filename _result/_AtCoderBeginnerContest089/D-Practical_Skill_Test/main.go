...
func main() {
	...
	for i := range ax {
		for j := range ax[i] {
			mem[ax[i][j]-1] = []int{i, j}
		}
	}

	sum := make([][]int, d)
	for i := range sum {
		sum[i] = make([]int, 1, h*w/d+1)
	}

	for res := 0; res < d; res++ {
		for i := 1; res+i*d < h*w; i++ {
			s := (i-1)*d + res
			t := i*d + res
			sum[res] = append(sum[res], abs(mem[s][0]-mem[t][0])+abs(mem[s][1]-mem[t][1])+sum[res][i-1])
		}
	}
