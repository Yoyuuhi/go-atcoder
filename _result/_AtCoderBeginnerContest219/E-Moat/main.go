...
func isCovered(ax, bx [][]int) bool {
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			if ax[i][j] == 1 && bx[i][j] != 1 {
				return false
			}
		}
	}
...
func hasHole(bx [][]int) bool {
	start := point{}
	mX := wfs1(bx, start, 4, 4, 3)
	for i := 0; i < 6; i++ {
		for j := 0; j < 6; j++ {
			if mX[i][j] == 0 {
				return true
...
func wfs(sX [][]int, start point, h, w, val int) {
	l := list.New()
	l.PushBack(start)
	lE := l.Front()

	sX[start.x][start.y] = val

	for lE != nil {
		now := lE.Value.(point)
		for _, v := range vectorsA {
			toX := now.x + v.x
			toY := now.y + v.y

			if toX < h && toX >= 0 && toY < w && toY >= 0 && sX[toX][toY] == 1 {
				sX[toX][toY] = val
				l.PushBack(point{x: toX, y: toY})
			}
		}
...
func wfs1(sX [][]int, start point, h, w, val int) [][]int {
	l := list.New()
	l.PushBack(start)
	lE := l.Front()
	...
	for lE != nil {
		...
		for _, v := range vectorsA {
			...
			if toX < h+2 && toX >= 0 && toY < w+2 && toY >= 0 && mX[toX][toY] == 0 {
				mX[toX][toY] = val
				l.PushBack(point{x: toX, y: toY})
			}
		}
