...
func main() {
	...
	sorti(das)
	memNumIdx := map[int]int{}
	for i, num := range das {
		memNumIdx[num] = i
	}

	memBit := newbit(len(memA))
	memBit.add(memNumIdx[as[0]], mdiv(1, mpow(2, 0, mod)))

	ans := 0
	for i := 1; i < n; i++ {
		sum := memBit.sum(memNumIdx[as[i]])
		ans = madd(ans, mmul(sum, mpow(2, i-1, mod)))
		memBit.add(memNumIdx[as[i]], mdiv(1, mpow(2, i, mod)))
	}
	out(ans)
}
