...
func main() {
	...
	for i := 0; i < len(n); i++ {
		nums[i] = atoi(string(n[i]))
	}
	sorti(nums)

	for bits := 1; bits < (1<<len(n))-1; bits++ {
		ans = max(ans, simulate(nums, bits))
	}
	out(ans)
}

func simulate(nums []int, bits int) int {
	leftNum := ""
	rightNum := ""
	...
	for i := len(nums) - 1; i >= 0; i-- {
		if (bits>>uint64(i))&1 == 1 {
			...
			rightNum += itoa(nums[i])
		}
	}
	return atoi(leftNum) * atoi(rightNum)
}
