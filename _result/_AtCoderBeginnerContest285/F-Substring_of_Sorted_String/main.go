...
loopq:
	for i := 0; i < q; i++ {
		...
		case 1:
			...
			if x > 0 {
				if c >= ss[x-1] {
					if b.rangesum(x, x+1) == 0 {
						b.add(x, 1)
					}
				} else {
					if b.rangesum(x, x+1) == 1 {
						b.add(x, -1)
					}
				}
			}
			if x+1 < n {
				if ss[x+1] >= c {
					if b.rangesum(x+1, x+2) == 0 {
						b.add(x+1, 1)
					}
				} else {
					if b.rangesum(x+1, x+2) == 1 {
						b.add(x+1, -1)
					}
				}
			}
		...
		case 2:
			...
			if b.rangesum(l+1, r+1) != r-l {
				out("No")
				continue
			}
			lc := ss[l]
			rc := ss[r]
			for j := 1; byte(j) < rc[0]-lc[0]; j++ {
				c := string(lc[0] + byte(j))
				if memb[c].rangesum(l, r+1) != memb[c].sum(n-1) {
					out("No")
					continue loopq
				}
			}
