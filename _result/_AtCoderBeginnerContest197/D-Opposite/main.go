...
func main() {
	...
	o := epoint{(x0 + xn2) / 2, (y0 + yn2) / 2}
	v1 := epoint{x0 - o.x, y0 - o.y}

	deg := 2 * math.Pi / n
	deg1 := math.Atan(v1.y / v1.x)

	if (deg1 > 0 && v1.y < 0) || (deg1 < 0 && v1.y > 0) || (deg1 == 0 && v1.x < 0) {
		deg1 += math.Pi
	}
	...
	l := math.Sqrt(v1.x*v1.x + v1.y*v1.y)
	out(o.x+l*math.Cos(deg2), o.y+l*math.Sin(deg2))
}
