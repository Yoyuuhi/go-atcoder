...
func main() {
	...
	for i := 0; i < n; i++ {
		x, c := ni2()
		mapCX[c] = append(mapCX[c], x)
	}
	cs := make([]int, len(mapCX))
	idx := 0
	for c := range mapCX {
		cs[idx] = c
		idx++
	...
	mem := make([]memc, len(cs))
	for i, c := range cs {
		sorti(mapCX[c])
		mem[i] = memc{
			mapCX[c][0],
			mapCX[c][len(mapCX[c])-1],
			mapCX[c],
		}
	}

	dp := make([]map[int]int, len(cs)+1)
	...
	for i := 0; i < len(cs); i++ {
		...
		nows := []int{-inf, inf, -inf, inf}
		for now := range dp[i] {
			if now <= l {
				nows[0] = max(nows[0], now)
			}
			if now > l {
				nows[1] = min(nows[1], now)
			}
			if now <= r {
				nows[2] = max(nows[2], now)
			}
			if now > r {
				nows[3] = min(nows[3], now)
			}
		}
		...
		for _, now := range nows {
			...
			for _, x := range mem[i].xs {
				var d int
				switch true {
				case (l >= now && r >= now) || (l <= now && r <= now):
					var f int
					...
					d = abs(f-now) + abs(f-x)
				default:
					d1 := abs(l-now) + abs(l-r) + abs(r-x)
					d2 := abs(r-now) + abs(l-r) + abs(l-x)
					d = min(d1, d2)
				}
				if v, e := dp[i+1][x]; !e || v > d+dp[i][now] {
					dp[i+1][x] = d + dp[i][now]
				}
			}
		}
