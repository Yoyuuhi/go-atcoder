...
func main() {
	...
	for bits := 1; bits < (1<<(n-1) - 1); bits++ {
		ans = min(ans, simulate(as, bits))
	}
	out(ans)
}

func simulate(as []int, bits int) int {
	bs := make([][]int, bitlen(bits)+1)

	ib := 0
	bs[0] = append(bs[0], as[0])
	for i := 1; i < len(as); i++ {
		if (bits>>uint64(i-1))&1 == 1 {
			ib++
		}
		bs[ib] = append(bs[ib], as[i])
	}
	return calc(bs)
}

func calc(bs [][]int) int {
	cs := make([]int, len(bs))
	for i, b := range bs {
		c := 0
		for _, bb := range b {
