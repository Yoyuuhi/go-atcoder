...
func main() {
	...
	dp := make([][4]int, h+1)
	for i := 1; i < h+1; i++ {
		dp[i] = [4]int{inf, inf, inf, inf}
	}
	dp[0] = [4]int{0, inf, inf, inf}
	...
	for i := 0; i <= h; i++ {
		for s, v := range dp[i] {
			...
			if !mu {
				dp[i+1][(s&1)*2] = min(dp[i+1][(s&1)*2], v)
				dp[i+1][(s&1)*2+1] = min(dp[i+1][(s&1)*2+1], v+1)
			} else {
				dp[i+1][(s&1)*2+sw] = min(dp[i+1][(s&1)*2+sw], v+sw)
			}
		}
	}
	ans := inf
...
func must(i int, s int, ax [][]int) (bool, int) {
	mu := false
	sw := 2
	if i == 0 {
		return false, 2
...
func calcConnect(s int, i int, ax [][]int) []bool {
	s1 := nthbit(s, 1)
	s2 := s & 1
	ans := make([]bool, w)
	for j := 0; j < w; j++ {
		connected := false
		if j-1 >= 0 && (ax[i-1][j-1]+s2)%2 == (ax[i-1][j]+s2)%2 {
			connected = true
		}
		if j+1 < w && (ax[i-1][j+1]+s2)%2 == (ax[i-1][j]+s2)%2 {
			connected = true
		}
		if i > 1 && (ax[i-2][j]+s1)%2 == (ax[i-1][j]+s2)%2 {
			connected = true
		}
		ans[j] = connected
	}
