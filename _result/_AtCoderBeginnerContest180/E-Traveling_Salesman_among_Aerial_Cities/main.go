...
func main() {
	...
	for i := 1; i < n; i++ {
		x, y, z := xyzs[i].x, xyzs[i].y, xyzs[i].z
		dp[i][1<<i] = abs(x-xyzs[0].x) + abs(y-xyzs[0].y) + max(0, z-xyzs[0].z)
		l.PushBack(posBit{i, 1 << i})
	}
	...
	for e != nil {
		...
		for i := 0; i < n; i++ {
			...
			val := dp[pos][bit] + abs(xyzs[i].x-xyzs[pos].x) + abs(xyzs[i].y-xyzs[pos].y) + max(0, xyzs[i].z-xyzs[pos].z)
			if v, e := dp[i][bit|(1<<i)]; !e || val < v {
				dp[i][bit|(1<<i)] = val
				if !e {
					l.PushBack(posBit{i, bit | (1 << i)})
				}
			}
		}
		e = e.Next()
