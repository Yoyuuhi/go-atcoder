...
func main() {
	...
	for i := i0; i < n; i++ {
		...
		l := d - r
		if l != 0 {
			ans = madd(ans, mmul(mpow(2, n-1-i, mod), mmul(2, mmul(r-l+1, mpow(2, d-2, mod)))))
		} else {
			ans = madd(ans, mmul(mpow(2, n-1-i, mod), mmul(2, madd(mmul(r-l-1, mpow(2, d-2, mod)), mmul(2, mpow(2, d-1, mod))))))
		}
	}
	out(ans)
}
