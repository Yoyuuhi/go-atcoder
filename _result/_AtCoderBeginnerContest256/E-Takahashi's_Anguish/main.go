...
func main() {
	...
	uf := newUnionFind(n)
	for i := 0; i < n; i++ {
		edges[i] = edge{to: xs[i] - 1, cost: cs[i]}
		uf.unite(i, xs[i]-1)
	}

	passedRoot := map[int]bool{}
	passedNode := map[int]int{}

	ans := 0
	for i := 0; i < n; i++ {
		if passedRoot[uf.root(i)] {
			continue
		}
		passedRoot[uf.root(i)] = true

		now := i
		tempAns := inf
		for passedNode[now] < 3 {
			passedNode[now] += 1
			if passedNode[now] == 2 {
				tempAns = min(tempAns, edges[now].cost)
			}
			now = edges[now].to
		}
		if tempAns != inf {
