...
func main() {
	...
	for i := 1; i <= ns[0]-2; i++ {
		matrix[0][0] = i
		for j := 1; j <= ns[0]-i-1; j++ {
			matrix[1][0] = j
			matrix[2][0] = ns[0] - i - j

			for k := 1; k <= ns[1]-2; k++ {
				matrix[0][1] = k
				for l := 1; l <= ns[1]-k-1; l++ {
					matrix[1][1] = l
					matrix[2][1] = ns[1] - k - l

					matrix[0][2] = ns[3] - matrix[0][0] - matrix[0][1]
					matrix[1][2] = ns[4] - matrix[1][0] - matrix[1][1]
					matrix[2][2] = ns[5] - matrix[2][0] - matrix[2][1]

					if matrix[0][2] > 0 && matrix[1][2] > 0 && matrix[2][2] > 0 && matrix[0][2]+matrix[1][2]+matrix[2][2] == ns[2] {
						ans += 1
					}
				}
			}
