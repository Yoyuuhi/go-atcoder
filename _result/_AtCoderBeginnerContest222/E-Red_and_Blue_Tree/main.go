...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, idx: i})
		edges[t] = append(edges[t], edge{to: s, idx: i})
	}
	memETimes = make([]int, n-1)

	for i := 0; i < m-1; i++ {
		now := as[i] - 1
		t := as[i+1] - 1
		graphDfs(edges, now, t, -1, map[int]bool{})
	}

	sumRB := 0
	for _, t := range memETimes {
		sumRB += t
	}
	k = abs(k)
	if (sumRB == 0 && k != 0) || (sumRB+k)%2 != 0 {
		out(0)
		return
	}
	r := (sumRB + k) / 2
	...
	for i := 0; i < n-1; i++ {
		for k, v := range dp[i] {
			if k+memETimes[i] <= r {
				dp[i+1][k+memETimes[i]] = madd(v, dp[i+1][k+memETimes[i]])
			}
			dp[i+1][k] = madd(v, dp[i+1][k])
		}
	}
	out(dp[n-1][r])
}

func graphDfs(edges [][]edge, now int, t int, idx int, passed map[int]bool) bool {
	if now == t {
		if idx != -1 {
			memETimes[idx] += 1
		}
	...
	for _, edge := range edges[now] {
		if !passed[edge.to] {
			ans = ans || graphDfs(edges, edge.to, t, edge.idx, passed)
		}
	}
	if ans && idx != -1 {
		memETimes[idx] += 1
