...
func main() {
	...
	for i := 0; i < m; i++ {
		memOrdUser := calOrder(winMem, n)

		for j := 1; j < 2*n; j += 2 {
			p1 := string(aM[memOrdUser[j]-1][i])
			p2 := string(aM[memOrdUser[j+1]-1][i])

			switch true {
			case p1 == "G" && p2 == "C":
				winMem[memOrdUser[j]] += 1
			case p1 == "G" && p2 == "P":
				winMem[memOrdUser[j+1]] += 1
			case p1 == "C" && p2 == "G":
				winMem[memOrdUser[j+1]] += 1
			case p1 == "C" && p2 == "P":
				winMem[memOrdUser[j]] += 1
...
func calOrder(winMem map[int]int, n int) map[int]int {
	winNumUserMem := map[int]*IntHeap{}
	for i := 1; i <= 2*n; i++ {
		if winNumUserMem[winMem[i]] == nil {
			winNumUserMem[winMem[i]] = newIntHeap(asc)
		}
		winNumUserMem[winMem[i]].Push(i)
	}

	keys := make([]int, len(winNumUserMem))
	ind := 0
	for k, _ := range winNumUserMem {
		keys[ind] = k
		ind++
	}

	sortir(keys)
	ansOrdUser := map[int]int{}
	i := 1
	for _, k := range keys {
		for !winNumUserMem[k].IsEmpty() {
			s := winNumUserMem[k].Pop()
			ansOrdUser[i] = s
			i++
		}
	}
