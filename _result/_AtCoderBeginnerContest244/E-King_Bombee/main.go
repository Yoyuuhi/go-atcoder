	...
	for i := 0; i < k; i++ {
		for j := 0; j < n; j++ {
			...
			for _, e := range edges[j] {
				if e.to == x {
					dp[i+1][e.to][0] = madd(dp[i+1][e.to][0], dp[i][j][1])
					dp[i+1][e.to][1] = madd(dp[i+1][e.to][1], dp[i][j][0])
				} else {
					dp[i+1][e.to][0] = madd(dp[i+1][e.to][0], dp[i][j][0])
					dp[i+1][e.to][1] = madd(dp[i+1][e.to][1], dp[i][j][1])
				}
			}
		}
	}
