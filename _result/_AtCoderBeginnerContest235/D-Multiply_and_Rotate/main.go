...
func main() {
	...
	a, n = ni2()
	simulate(1, 0, make(map[int]int, n))
	if ans != inf {
		out(ans)
	} else {
		out(-1)
...
func simulate(now, tempAns int, passed map[int]int) {
	if now == n {
		ans = min(ans, tempAns)
		return
	}

	if (tempAns >= passed[now] && passed[now] != 0) || len(itoa(now)) > len(itoa(n)) {
		return
	}
	passed[now] = tempAns

	simulate(now*a, tempAns+1, passed)
	if len(itoa(now)) > 1 && now%10 != 0 {
		s := itoa(now)
		s = string(s[len(s)-1]) + s[:len(s)-1]
		simulate(atoi(s), tempAns+1, passed)
	}
}
