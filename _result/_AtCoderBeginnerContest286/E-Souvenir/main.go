...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		out(score[u][v], sos[u][v])
	}
}

func (g *graph) eFloydWarshall() ([][]int, bool) {
	...
	for k := 0; k < g.size; k++ {
		for i := 0; i < g.size; i++ {
			for j := 0; j < g.size; j++ {
				...
				if score[i][k]+score[k][j] < score[i][j] {
					score[i][j] = score[i][k] + score[k][j]
					sos[i][j] = sos[i][k] + sos[k][j] - as[k]
				}
				if score[i][k]+score[k][j] == score[i][j] {
					sos[i][j] = max(sos[i][j], sos[i][k]+sos[k][j]-as[k])
				}
			}
		}
	}
