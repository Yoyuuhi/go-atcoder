...
func searchR(sM []string) bool {
	for i := 0; i < len(sM); i++ {
		temp := make([]int, len(sM))
		for j := 0; j < len(sM); j++ {
			if string(sM[i][j]) == "#" {
				temp[j]++
			}
...
func searchD(sM []string) bool {
	temp := make([][]int, len(sM))
	for i := 0; i < len(sM); i++ {
		temp[i] = make([]int, len(sM))
	}
	...
	for i := 0; i < len(sM); i++ {
		for j := 0; j < len(sM); j++ {
			...
			if (i+5 > len(sM)-1 && j < 6) || (j+5 > len(sM)-1 && i < 6) {
				continue
			}
			var minus int
			if i > 5 && j > 5 {
...
func searchDR(sM []string) bool {
	...
	for i := 0; i < len(sM); i++ {
		for j := 0; j < len(sM); j++ {
			...
			if (i < 6 && j < 6) || (i+5 > len(sM)-1 && j+5 > len(sM)-1) {
				continue
			}
			var minus int
			if i > 5 && j+6 < len(sM) {
