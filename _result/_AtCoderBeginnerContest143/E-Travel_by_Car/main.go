...
func (g *graph) eFloydWarshall() [][][2]int {

	score := make([][][2]int, g.size)
	for i := 0; i < g.size; i++ {
		score[i] = make([][2]int, g.size)
		for j := 0; j < g.size; j++ {
			if i == j {
				score[i][j] = [2]int{0, l}
			} else {
				score[i][j] = [2]int{g.defaultScore, 0}
			}
		}
		for _, edge := range g.edges[i] {
			score[i][edge.to] = [2]int{0, l - edge.cost}
		}
	}
	for k := 0; k < g.size; k++ {
		for i := 0; i < g.size; i++ {
			for j := 0; j < g.size; j++ {
				if score[i][k][0] == g.defaultScore || score[k][j][0] == g.defaultScore {
					continue
				}
				if score[k][j][0] == 0 && l-score[k][j][1] <= score[i][k][1] {
					if score[i][k][0]+score[k][j][0] < score[i][j][0] {
						score[i][j][0] = score[i][k][0] + score[k][j][0]
						score[i][j][1] = score[i][k][1] - (l - score[k][j][1])
					} else if score[i][k][0]+score[k][j][0] == score[i][j][0] {
						score[i][j][1] = max(score[i][j][1], score[i][k][1]-(l-score[k][j][1]))
					}
				}
			}
		}
	...
	for k := 0; k < g.size; k++ {
		for i := 0; i < g.size; i++ {
			for j := 0; j < g.size; j++ {
				...
				if score[i][k][0]+score[k][j][0]+1 < score[i][j][0] {
					score[i][j][0] = score[i][k][0] + score[k][j][0] + 1
					score[i][j][1] = score[k][j][1]
				} else if score[i][k][0]+score[k][j][0]+1 == score[i][j][0] {
					score[i][j][1] = max(score[i][j][1], score[k][j][1])
				}
			}
		}
	}
