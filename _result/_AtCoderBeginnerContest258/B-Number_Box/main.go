...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			ans = max(ans, simulate(aMat, i, j, n))
		}
	}
	out(ans)
}

func simulate(aMat []string, i, j, n int) int {
	var ansStr string
	var ansInt int

	direct := append([][]int{}, []int{-1, 0}, []int{1, 0}, []int{0, -1}, []int{0, 1}, []int{1, 1}, []int{1, -1}, []int{-1, 1}, []int{-1, -1})

	for _, dir := range direct {
		I := i
		J := j
		ansStr = string(aMat[I][J])
		for count := 1; count < n; count++ {
			I, J = overFlow(I+dir[0], J+dir[1], n)
			ansStr += string(aMat[I][J])
		}
		ansInt = max(ansInt, atoi(ansStr))
	}
	return ansInt
}

func overFlow(x, y, n int) (int, int) {
	switch x {
	case -1:
		x = n - 1
	case n:
