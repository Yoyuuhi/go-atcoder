...
func main() {
	...
	count := 0
	for {
		next := bs(2*n, 1, func(c int) bool {
			return sums[c]-sums[now] >= x
		})
		memc[now] = [3]int{next - now, count, next % n}

		if v, e := memc[next%n]; e {
			cycle = memc[now][1] + 1 - v[1]
			break
		}
		count++
		now = next % n
	...
	memAns := make([]int, len(memc)+1)
	idx := 1
	now = 0
	passed := map[int]bool{}
	for !passed[now] {
		passed[now] = true
		memAns[idx] = memc[now][0]
		now = memc[now][2]
		idx++
	}
	lenOffset := len(memc) - cycle

	for i := 0; i < q; i++ {
		k := ni()
		if k > len(memc) {
			res := (k - len(memc)) % cycle
			if res == 0 {
				res += cycle
			}
			out(memAns[lenOffset+res] + offset)
		} else {
			out(memAns[k] + offset)
		}
	}
