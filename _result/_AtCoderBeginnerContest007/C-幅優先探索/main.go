...
func main() {
	...
	ps := list.New()
	ps.PushBack(point{x: sy, y: sx})
	psE := ps.Front()

	passed := map[int]map[int]int{}
	passed[sy] = map[int]int{}
	passed[sy][sx] = 0
	for psE != nil {
		now := psE.Value.(point)
		...
		if _, e := passed[now.x+1][now.y]; now.x+1 < r && string(cx[now.x+1][now.y]) == "." && !e {
			passed[now.x+1][now.y] = passed[now.x][now.y] + 1
			ps.PushBack(point{x: now.x + 1, y: now.y})
		}
		if _, e := passed[now.x-1][now.y]; now.x-1 >= 0 && string(cx[now.x-1][now.y]) == "." && !e {
			passed[now.x-1][now.y] = passed[now.x][now.y] + 1
			ps.PushBack(point{x: now.x - 1, y: now.y})
		}
		if _, e := passed[now.x][now.y+1]; now.y+1 < c && string(cx[now.x][now.y+1]) == "." && !e {
			passed[now.x][now.y+1] = passed[now.x][now.y] + 1
			ps.PushBack(point{x: now.x, y: now.y + 1})
		}
		if _, e := passed[now.x][now.y-1]; now.y-1 >= 0 && string(cx[now.x][now.y-1]) == "." && !e {
			passed[now.x][now.y-1] = passed[now.x][now.y] + 1
			ps.PushBack(point{x: now.x, y: now.y - 1})
		}
		psE = psE.Next()
