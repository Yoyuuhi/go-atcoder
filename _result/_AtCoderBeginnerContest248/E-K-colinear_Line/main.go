...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			...
			v1 := point{ps[i].x - ps[j].x, ps[i].y - ps[j].y}

			count := 2
			ks := make([]int, 0, n-j)
			for k := j + 1; k < n; k++ {
				if passed[i][k] || passed[j][k] {
					continue
				}
				v2 := point{ps[i].x - ps[k].x, ps[i].y - ps[k].y}
			...
			for i2 := 0; i2 < len(ks); i2++ {
				for j2 := i2 + 1; j2 < len(ks); j2++ {
					passed[ks[i2]][ks[j2]] = true
				}
			}
