...
func main() {
	...
	ri := 1
	ans := 0
	if vl1[li][0] == vl2[ri][0] {
		ans += min(vl1[li][1], vl2[ri][1])
	}
	for li < n1 || ri < n2 {
		switch vl1[li][1] <= vl2[ri][1] {
		case true:
			for li+1 <= n1 && vl1[li][1] <= vl2[ri][1] {
				li++
				if vl1[li][0] == vl2[ri][0] {
					ans += max(0, min(vl1[li][1], vl2[ri][1])-vl1[li-1][1])
				}
			}

		case false:
			for ri+1 <= n2 && vl1[li][1] >= vl2[ri][1] {
				ri++
				if vl1[li][0] == vl2[ri][0] {
					ans += max(0, min(vl1[li][1], vl2[ri][1])-vl2[ri-1][1])
				}
			}
		}
	}
