...
func checkCircle(edges [][]edge) bool {
	passed := make(map[int]bool, len(edges))
	for i, _ := range edges {
		if passed[i] {
			continue
		}
		if !graphDfs(edges, i, i, passed) {
			return false
		}
	}
	return true
...
func graphDfs(edges [][]edge, now int, prev int, passed map[int]bool) bool {
	passed[now] = true
	for _, edge := range edges[now] {
		if edge.to != prev {
			...
			graphDfs(edges, edge.to, now, passed)
		}
	}
	return true
}
