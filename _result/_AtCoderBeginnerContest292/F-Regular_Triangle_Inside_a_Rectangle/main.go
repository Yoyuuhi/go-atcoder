...
func main() {
	...
	ans := bsfl(1, 2000, 1000, func(c float64) bool {
		te := bsfl(30, 0, 1000, func(c1 float64) bool {
			return math.Cos(c1/180.0*math.Pi)*c <= float64(b)
		})
		if te == -1 {
			return false
		}
		return math.Sin((te+60)/180.0*math.Pi)*c <= float64(a)
	})
	out(ans)
}
