...
func simulate(a, s int) bool {
	if a == 0 {
		return true
	}

	if bitlen(a)+1 > bitlen(s) {
		return false
	}

	aBits := make([]int, bitlen(s))
	memOnes := make(map[int]bool, bitlen(a))
	for i := 0; i < bitlen(a); i++ {
		aBits[i+1] = nthbit(a, i)
		if nthbit(a, i) == 1 {
			memOnes[i] = true
		}
	}

	sBits := make([]int, bitlen(s))
	for i := 0; i < bitlen(s); i++ {
		sBits[i] = nthbit(s, i)
	}
