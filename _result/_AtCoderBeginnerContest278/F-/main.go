...
var ss []string
var mem map[string]map[int]bool
...
func main() {
	...
	for i := 0; i+1 < n; i++ {
		for j := i + 1; j < n; j++ {
			...
			if b[len(b)-1] == a[0] {
				edges[j] = append(edges[j], edge{to: i})
			}
		}
	}
	...
	mem = map[string]map[int]bool{}
	for i := "a"; i <= "z"; i = string(i[0] + 1) {
		passed[i] = map[int]bool{}
		mem[i] = map[int]bool{}
	}
	for i := 0; i < n; i++ {
		if graphDfs(edges, i, 0, 0, passed) {
			out("First")
			return
		}
	}
...
func graphDfs(edges [][]edge, now int, color int, route int, passed map[string]map[int]bool) bool {
	route = route | (1 << now)
	if passed[string(ss[now][len(ss[now])-1])][route] {
		return mem[string(ss[now][len(ss[now])-1])][route]
	}
	passed[string(ss[now][len(ss[now])-1])][route] = true
	var poss bool
	if color == 1 {
		poss = true
	}
	...
	for _, edge := range edges[now] {
		if route&(1<<edge.to) == 0 {
			next++
			if color == 1 {
				poss = poss && graphDfs(edges, edge.to, (color+1)%2, route, passed)
			} else {
				poss = poss || graphDfs(edges, edge.to, (color+1)%2, route, passed)
			}
		}
	...
	if next == 0 && color == 0 {
		mem[string(ss[now][len(ss[now])-1])][route] = true
		return true
	}
	if next == 0 && color == 1 {
		mem[string(ss[now][len(ss[now])-1])][route] = false
		return false
	}
	mem[string(ss[now][len(ss[now])-1])][route] = poss
	return poss
}
