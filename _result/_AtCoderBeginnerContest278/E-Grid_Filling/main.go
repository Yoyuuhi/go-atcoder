...
func main() {
	...
	for i := 0; i < h1+1; i++ {
		mem[i] = make([][]int, w1+1)
		mem[i][0] = make([]int, n+1)
	}
	for j := 1; j < w1+1; j++ {
		mem[0][j] = make([]int, n+1)
	}

	for i := 1; i < h1+1; i++ {
		for j := 1; j < w1+1; j++ {
			mem[i][j] = make([]int, n+1)
			mem[i][j][ax[i-1][j-1]] += 1
		}
	}
	for i := 1; i < h1+1; i++ {
		for j := 1; j < w1+1; j++ {
			for k, v := range mem[i][j-1] {
				mem[i][j][k] += v
			}
		}
	}
	...
	for i := 0; i+h <= h1; i++ {
		ans := make([]int, w1-w+1)
		for j := 0; j+w <= w1; j++ {
			mem0 := copySlice(mem[i+h][j+w])
			mem1 := copySlice(mem[i][j+w])
			mem2 := copySlice(mem[i+h][j])
			mem3 := copySlice(mem[i][j])
			count := 0
