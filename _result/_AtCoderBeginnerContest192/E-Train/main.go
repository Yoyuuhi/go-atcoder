...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, cost: c, k: k})
		edges[t] = append(edges[t], edge{to: s, cost: c, k: k})
	}
...
func calcToT(s int, cost int, k int) int {
	if s%k == 0 {
		return s + cost
	} else {
		return (s/k+1)*k + cost
...
func (g *graph) dijkstra(start int) []int {
	...
	for !que.IsEmpty() {
		...
		for _, edge := range g.edges[st.node] {
			newScore := calcToT(st.score, edge.cost, edge.k)
