	...
	for i := 1; i <= r; i++ {
		for j := 1; j <= c; j++ {
			for s := 0; s <= 3; s++ {
				...
				dp[i][j][3] = dp[i][j-1][3]
				if rcv[i][j] > 0 {
					dp[i][j][1] = max(dp[i][j-1][1], dp[i][j-1][0]+rcv[i][j])
					dp[i][j][2] = max(dp[i][j-1][2], dp[i][j-1][1]+rcv[i][j])
					dp[i][j][3] = max(dp[i][j-1][3], dp[i][j-1][2]+rcv[i][j])
				}
				...
				if rcv[i][j] > 0 {
					dp[i][j][1] = max(dp[i][j][1], dp[i-1][j][0]+rcv[i][j])
					dp[i][j][1] = max(dp[i][j][1], dp[i-1][j][1]+rcv[i][j])
					dp[i][j][1] = max(dp[i][j][1], dp[i-1][j][2]+rcv[i][j])
					dp[i][j][1] = max(dp[i][j][1], dp[i-1][j][3]+rcv[i][j])
				}
			}
		}
	}
