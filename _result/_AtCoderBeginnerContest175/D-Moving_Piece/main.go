...
func main() {
	...
	ans := -inf
	for r := range rs {
		ans = max(ans, calc(r, n, k, edges, cs))
	}
	out(ans)
}

func calc(r, n, k int, edges []edge, cs []int) int {
	now := r
	...
	for {
		to := edges[now].to
		if _, e := memOrder[to]; e && l == 0 {
			ls = memOrder[to]
			l = memOrder[now] - memOrder[to] + 1
			break
		}
		memOrder[to] = memOrder[now] + 1
		now = to
	}

	memVal := make([]int, ls+2*l)
	now = r
	idx := 1
	for idx < ls+2*l {
		to := edges[now].to
		memVal[idx] += cs[to]
		memVal[idx] += memVal[idx-1]
		idx++
		now = to
	}
	lVal := memVal[ls+l] - memVal[ls]
