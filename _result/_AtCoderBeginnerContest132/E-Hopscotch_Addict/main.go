...
func main() {
	...
	ans[s0] = [3]int{0, inf, inf}

	cities := list.New()
	cities.PushBack([]int{s0, 0})
	e := cities.Front()
	...
	for e != nil {
		...
		for _, edge := range edges[now] {
			if ans[now][res]+1 >= ans[edge.to][(res+1)%3] {
				continue
			}
			ans[edge.to][(res+1)%3] = ans[now][res] + 1

			cities.PushBack([]int{edge.to, (res + 1) % 3})
		}
		e = e.Next()
	}
