...
func main() {
	...
	for i := 1; i <= k; i++ {
		var nr, ng int
		ng = cf.combination(k-1, i-1)
		ans := 0
		// lr
		nr = cf.combination(n-k-1, i)
		ans = madd(ans, mmul(nr, ng))

		// l, r
		nr = cf.combination(n-k-1, i-1)
		ans = madd(ans, mmul(2, mmul(nr, ng)))

		// center
		if i >= 2 {
			nr = cf.combination(n-k-1, i-2)
			ans = madd(ans, mmul(nr, ng))
		}
