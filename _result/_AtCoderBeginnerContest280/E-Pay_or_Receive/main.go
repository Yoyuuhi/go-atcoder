...
var uf *unionFind
var score map[int]map[int]int
var memInf map[int]bool
...
func main() {
	...
	score = map[int]map[int]int{}
	for root := range roots {
		score[root] = map[int]int{}
		bfs(root, edges)
	}
	...
	for i := 0; i < q; i++ {
		...
		r := uf.root(x)
		out(score[r][x] - score[r][y])
	}
}

func bfs(s int, edges [][]edge) {
	cities := list.New()
	cities.PushBack(s)
	score[s][s] = 0
	...
	for e != nil {
		...
		for _, edge := range edges[now] {
			if v, e := score[s][edge.to]; e && v != score[s][now]+edge.cost {
				memInf[s] = true
				return
			} else if e && v == score[s][now]+edge.cost {
				continue
			}
			score[s][edge.to] = score[s][now] + edge.cost
