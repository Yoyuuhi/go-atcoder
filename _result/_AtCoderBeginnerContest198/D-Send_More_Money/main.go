...
func main() {
	...
	memChars := map[string]bool{}
	for i := 0; i < len(s1); i++ {
		memChars[string(s1[i])] = true
	}
	for i := 0; i < len(s2); i++ {
		memChars[string(s2[i])] = true
	}
	for i := 0; i < len(s3); i++ {
		memChars[string(s3[i])] = true
	}

	if len(memChars) > 10 || len(s3) < len(s1) || len(s3) < len(s2) {
		out("UNSOLVABLE")
		return
	}

	chars := make([]string, len(memChars))
	i := 0
	for k, _ := range memChars {
		chars[i] = k
		i++
	}

	simulate(s1, s2, s3, chars, 0, map[int]bool{}, map[string]int{})
...
func simulate(s1, s2, s3 string, chars []string, ind int, used map[int]bool, charNum map[string]int) {
	if ans {
		return
	}

	if ind == len(chars) {
		justify(s1, s2, s3, charNum)
		return
	}

	for i := 0; i < 10; i++ {
		if used[i] || ((chars[ind] == string(s1[0]) || chars[ind] == string(s2[0]) || chars[ind] == string(s3[0])) && i == 0) {
			continue
		}
		charNum[string(chars[ind])] = i
		used[i] = true
		simulate(s1, s2, s3, chars, ind+1, used, charNum)
		used[i] = false
	}
}

func justify(s1, s2, s3 string, charNum map[string]int) {
	n1 = convToNum(s1, charNum)
	n2 = convToNum(s2, charNum)
	n3 = convToNum(s3, charNum)
...
func convToNum(s string, charNum map[string]int) int {
	ansS := ""
	for i := 0; i < len(s); i++ {
		ansS += itoa(charNum[string(s[i])])
	}
	return atoi(ansS)
}
