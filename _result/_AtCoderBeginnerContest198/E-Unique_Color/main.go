...
func main() {
	...
	ansMem = make([]bool, n)
	graphDfs(edges, 0, map[int]bool{}, map[int]bool{})

	for ans, v := range ansMem {
		if v {
			out(ans + 1)
		}
	}
...
func graphDfs(edges [][]edge, now int, passed map[int]bool, passedColor map[int]bool) {
	passed[now] = true
	eC := true
	if !passedColor[uf.root(now)] {
		ansMem[now] = true
		eC = false
	}
	passedColor[uf.root(now)] = true

	for _, edge := range edges[now] {
		if !passed[edge.to] {
			graphDfs(edges, edge.to, passed, passedColor)
		}
	}
	if !eC {
		passedColor[uf.root(now)] = false
	}
}
