...
func main() {
	...
	leftI := 0
	rightI := len(s) - 1
	for leftI < len(s) && string(s[leftI]) == "a" {
		leftI++
	}
	for rightI >= 0 && string(s[rightI]) == "a" {
		rightI--
	}

	if leftI+1 > len(s)-rightI {
		out("No")
		return
	}

	if simulate(s, leftI, rightI) {
		out("Yes")
		return
	} else {
		out("No")
...
func simulate(s string, leftI, rightI int) bool {
	for leftI < rightI {
		if s[leftI] != s[rightI] {
			return false
		}
