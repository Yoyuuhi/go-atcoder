			...
			for _, c := range cs {
				...
				if valid(k + c) {
					if len(k) == 2 {
						...
						dp[i+1][string(k[1])+string(k[2])+c] = madd(dp[i+1][string(k[1])+string(k[2])+c], v)
					}
				}
			}
		}
...
func valid(s string) bool {
	...
	ss := make([]string, 4)
	ss[0] = s
	ss[1] = string(s[1]) + string(s[0]) + string(s[2]) + string(s[3])
	ss[2] = string(s[0]) + string(s[2]) + string(s[1]) + string(s[3])
	ss[3] = string(s[0]) + string(s[1]) + string(s[3]) + string(s[2])

	for _, s0 := range ss {
		if string(s0[:3]) == "AGC" || string(s0[1:]) == "AGC" {
			return false
		}
	}
	return true
