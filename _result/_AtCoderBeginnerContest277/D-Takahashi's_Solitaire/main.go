...
var memAns map[int]int
var memAN map[int]int
var memed map[int]bool
...
func main() {
	...
	for a, _ := range memAN {
		to := (a + 1) % m
		edges[a] = append(edges[a], edge{to: to})
	}

	memAns = map[int]int{}
	memed = map[int]bool{}
	for root, _ := range roots {
		graphDfs(edges, root, root*memAN[root], map[int]int{})
	}
	...
	for a, _ := range memAN {
		...
		graphDfs(edges, a, a*memAN[a], map[int]int{})
	}

	ans := 0
	for _, v := range memAns {
		ans = max(ans, v)
	}
	out(sum - ans)
}

func graphDfs(edges map[int][]edge, now, val int, passed map[int]int) {
	memAns[now] = max(memAns[now], val)
	passed[now] = 1
	memed[now] = true

	for _, edge := range edges[now] {
		if passed[edge.to] != 1 {
			graphDfs(edges, edge.to, val+edge.to*memAN[edge.to], passed)
		}
	}
}
