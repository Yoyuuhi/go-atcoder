...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, sw: a, cost: 1})
		edges[t] = append(edges[t], edge{to: s, sw: a, cost: 1})
	}
	...
	graph := newegraph(n, edges)
	dist := graph.editDijkstra(0)

	ans := inf
	ans = min(ans, dist[n-1][0])
	ans = min(ans, dist[n-1][1])
...
func newegraph(size int, edges [][]edge) *egraph {
	graph := &egraph{
		size:  size,
		edges: edges,
	}
	...
	graph.comps = []compFunc{
		func(p, q interface{}) int {
			if p.(estate).score[p.(estate).s] < q.(estate).score[q.(estate).s] {
				return -1
			} else if p.(estate).score[p.(estate).s] == q.(estate).score[q.(estate).s] {
				return 0
			}
			return 1
		},
...
func (g *egraph) editDijkstra(start int) [][]int {

	g.starts = []estate{{node: start, score: []int{0, inf}, s: 0}}
	...
	for _, start := range g.starts {
		score[start.node][0] = start.score[0]
		heap.Push(que, start)
	}

	for !que.IsEmpty() {
		st := heap.Pop(que).(estate)
		if st.score[st.s] > score[st.node][st.s] {
			continue
		}
		for _, edge := range g.edges[st.node] {
			if !memS[st.node] && (edge.sw+st.s)%2 != 1 {
				continue
			}

			newScore := st.score[st.s] + edge.cost
			if (edge.sw+st.s)%2 != 1 {
				if score[edge.to][(st.s+1)%2] > newScore {
					score[edge.to][(st.s+1)%2] = newScore
					heap.Push(que, estate{score: score[edge.to], node: edge.to, s: (st.s + 1) % 2})
				}
				continue
			}

			if score[edge.to][st.s] > newScore {
				score[edge.to][st.s] = newScore
				heap.Push(que, estate{score: score[edge.to], node: edge.to, s: st.s})
			}
		}
	}
	return score
