...
func main() {
	...
	mapResArr := map[int][]int{}
	for i := 0; i < n; i++ {
		dfs(mapResArr, i, []int{}, 0, as)
	}
...
func dfs(mapResArr map[int][]int, nowI int, passed []int, resSum int, as []int) {
	if finished {
		return
	}

	if _, e := mapResArr[(resSum+as[nowI])%200]; e {
		finished = true
		bS = mapResArr[(resSum+as[nowI])%200]
		cS = append(passed, nowI+1)
		return
	}
	mapResArr[(resSum+as[nowI])%200] = []int{}
	mapResArr[(resSum+as[nowI])%200] = append(mapResArr[(resSum+as[nowI])%200], passed...)
	mapResArr[(resSum+as[nowI])%200] = append(mapResArr[(resSum+as[nowI])%200], nowI+1)

	for i := nowI + 1; i <= len(as)-1; i++ {
		dfs(mapResArr, i, append(passed, nowI+1), (resSum+as[nowI])%200, as)
	}
}
