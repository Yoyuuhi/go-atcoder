...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, cost: b, a: a})
		edges[t] = append(edges[t], edge{to: s, cost: b, a: a})
		suma += a
	}
	cds = make([][]int, n)
	for i := 0; i < n; i++ {
	...
	score := graph.editDijkstra(0, min(s, suma), suma)

	for i := 1; i < n; i++ {
		ans := inf
		for _, a := range score[i] {
			ans = min(ans, a)
		}
		out(ans)
	}
...
func newegraph(size int, edges [][]edge) *egraph {
	...
	graph.comps = []compFunc{
		func(p, q interface{}) int {
			if p.(estate).score < q.(estate).score {
				return -1
			} else if p.(estate).score == q.(estate).score {
				return 0
			}
			return 1
		},
...
func (g *egraph) editDijkstra(start int, s int, suma int) [][]int {
	score := make([][]int, g.size)
	for i := 0; i < g.size; i++ {
		score[i] = make([]int, suma+1)
		for j := range score[i] {
			score[i][j] = inf
		}
	}
	que := newpq(g.comps)
	star := estate{node: start, score: 0, s: s}
	score[start][s] = 0
	heap.Push(que, star)

	for !que.IsEmpty() {
		st := heap.Pop(que).(estate)
		if st.score > score[st.node][st.s] {
			continue
		}
		for _, edge := range g.edges[st.node] {
			newScore := st.score + edge.cost
			newSilver := min(st.s-edge.a, suma)
			...
			if score[edge.to][newSilver] > newScore {
				...
				heap.Push(que, estate{score: newScore, node: edge.to, s: newSilver})
			}
		}

		newScore := st.score + cds[st.node][1]
		newSilver := min(st.s+cds[st.node][0], suma)
		if score[st.node][newSilver] > newScore {
			score[st.node][newSilver] = newScore
			heap.Push(que, estate{score: newScore, node: st.node, s: newSilver})
		}
	}
	return score
}
