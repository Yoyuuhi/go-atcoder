...
func main() {
	...
	wfs(ss, mem, point{ax, ay}, n, n)
	out(mem[bx][by])
}

func wfs(ss []string, mem [][]int, start point, h, w int) {
	l := list.New()
	l.PushBack(start)
	lE := l.Front()

	mem[start.x][start.y] = 0
	...
	for lE != nil {
		...
		for _, v := range vs {
			...
			for toX < h && toX >= 0 && toY < w && toY >= 0 && string(ss[toX][toY]) != "#" && (mem[toX][toY] == -1 || mem[toX][toY] > mem[now.x][now.y]) {
				if mem[toX][toY] == -1 || mem[toX][toY] > mem[now.x][now.y]+1 {
					mem[toX][toY] = mem[now.x][now.y] + 1
