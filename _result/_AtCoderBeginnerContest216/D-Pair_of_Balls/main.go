...
func main() {
	...
	topMap := map[int]int{}
	for i := 0; i < m; i++ {
		if topMap[es[i].Value.(int)] == 0 {
			topMap[es[i].Value.(int)] = i + 1
		} else {
			simulate(topMap, es, i)
		}
	}
...
func simulate(topMap map[int]int, es []*list.Element, i int) {
	i1 := topMap[es[i].Value.(int)] - 1
	i2 := i

	es[i1] = es[i1].Next()
	if es[i1] != nil {
		switch true {
		case topMap[es[i1].Value.(int)] != 0:
			simulate(topMap, es, i1)
		default:
			topMap[es[i1].Value.(int)] = i1 + 1
		}
	}
	es[i2] = es[i2].Next()
	if es[i2] != nil {
		switch true {
		case topMap[es[i2].Value.(int)] != 0:
			simulate(topMap, es, i2)
		default:
			topMap[es[i2].Value.(int)] = i2 + 1
		}
	}
}
