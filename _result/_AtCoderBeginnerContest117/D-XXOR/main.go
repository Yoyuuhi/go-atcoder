...
func main() {
	...
	dp := make([][2]int, 40)
	if hasbit(k, 39) {
		dp[39][1] = (n - ones[39]) * pow(2, 39)
		dp[39][0] = ones[39] * pow(2, 39)
	} else {
		dp[39][1] = ones[39] * pow(2, 39)
	}

	for i := 38; i >= 0; i-- {
		if hasbit(k, i) {
			dp[i][1] = max(dp[i][1], dp[i+1][1]+(n-ones[i])*pow(2, i))
			dp[i][0] = max(dp[i][0], dp[i+1][1]+ones[i]*pow(2, i))
			dp[i][0] = max(dp[i][0], dp[i+1][0]+(n-ones[i])*pow(2, i))
			dp[i][0] = max(dp[i][0], dp[i+1][0]+ones[i]*pow(2, i))
		} else {
			dp[i][1] = max(dp[i][1], dp[i+1][1]+ones[i]*pow(2, i))
			if dp[i+1][0] != 0 {
				dp[i][0] = max(dp[i][0], dp[i+1][0]+(n-ones[i])*pow(2, i))
				dp[i][0] = max(dp[i][0], dp[i+1][0]+ones[i]*pow(2, i))
			}
