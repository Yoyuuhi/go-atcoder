...
func main() {
	...
	rootsNum := map[int][]int{}
	for i := 0; i < n; i++ {
		rootsNum[uf.root(i)] = append(rootsNum[uf.root(i)], i)
	}
	color := make([]int, n)
	for i := 0; i < n; i++ {
		color[i] = -1
	...
	rNums := make([]int, len(rootsNum))
	ans := 0

	idx := 0
	for r := range rootsNum {
		a, b, c, e := bfs(r, edges, color, rootsNum)
		if !e {
			out(0)
			return
		}
	...
	sumrNums := make([]int, len(rootsNum))
	for i := len(rootsNum) - 1; i >= 0; i-- {
		sumrNums[i] = rNums[i]
		if i != len(rootsNum)-1 {
			sumrNums[i] += sumrNums[i+1]
		}
	}
	for i := 0; i+1 < len(rootsNum); i++ {
		ans += rNums[i] * sumrNums[i+1]
	}
	out(ans)
}

func bfs(front int, edges [][]edge, color []int, rootsNum map[int][]int) (int, int, int, bool) {
	cities := list.New()
	cities.PushBack(front)
	e := cities.Front()
	passed := map[int]bool{}
	passed[front] = true
	color[front] = 1
	...
	for e != nil {
		...
		for _, edge := range edges[now] {
			...
			passed[edge.to] = true
			color[edge.to] = abs(color[now] - 1)

			cities.PushBack(edge.to)
		}
		e = e.Next()
	}
	...
	for _, i := range rootsNum[front] {
		if color[i] == 1 {
			num += numC0 - len(edges[i])
		}
	}

	return numC1, numC0, num, true
}
