...
func main() {
	...
	for _, ab := range abs {
		x, y := ab.x, ab.y
		r1, r2 := memR[x][y][0], memR[x][y][1]
		mem2[x][r1][0] += 1
		if r2+1 < w {
			mem2[x][r2+1][0] -= 1
		}
		c1, c2 := memC[x][y][0], memC[x][y][1]
		mem2[c1][y][1] += 1
		if c2+1 < h {
			mem2[c2+1][y][1] -= 1
		}
