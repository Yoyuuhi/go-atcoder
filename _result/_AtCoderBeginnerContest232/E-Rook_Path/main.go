...
func main() {
	...
	for a := 0; a < k; a++ {
		...
		for i := 0; i < 2; i++ {
			for j := 0; j < 2; j++ {
				if j != 0 {
					next[i][0] = madd(next[i][0], mmul(dp[i][j], 1))
				}
				if j != 1 {
					next[i][1] = madd(next[i][1], mmul(dp[i][j], w-1))
				} else {
					next[i][1] = madd(next[i][1], mmul(dp[i][j], w-2))
				}

				if i != 0 {
					next[0][j] = madd(next[0][j], mmul(dp[i][j], 1))
				}
				if i != 1 {
					next[1][j] = madd(next[1][j], mmul(dp[i][j], h-1))
				} else {
					next[1][j] = madd(next[1][j], mmul(dp[i][j], h-2))
				}
			}
		}
		dp = next
