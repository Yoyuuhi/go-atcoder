...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		s--
		t--
		aEdges[s] = append(aEdges[s], edge{to: t})
		aEdges[t] = append(aEdges[t], edge{to: s})
	}

	simulate(0, make([]bool, n), map[int]int{})
	if ans {
		out("Yes")
	} else {
		out("No")
...
func simulate(now int, usedMemT []bool, rel map[int]int) {
	if ans {
		return
	}

	for i, _ := range usedMemT {
		if !usedMemT[i] {
			usedMemT[i] = true
			rel[now] = i

			if len(rel) == len(usedMemT) {
				sameGraph(rel)
			}

			simulate(now+1, usedMemT, rel)
...
func sameGraph(rel map[int]int) {
	for iA, edges := range tEdges {
	edgeCheck:
		for _, edge := range edges {
			for _, e := range aEdges[rel[iA]] {
				if e.to == rel[edge.to] {
					continue edgeCheck
				}
			}
