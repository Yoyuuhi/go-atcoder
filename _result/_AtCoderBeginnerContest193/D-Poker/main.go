...
func main() {
	...
	memt := map[int]int{}
	for i := 0; i < 4; i++ {
		mems[atoi(string(s[i]))] += 1
		memt[atoi(string(t[i]))] += 1
	}

	rest := map[int]int{}
	for i := 1; i <= 9; i++ {
		if k-mems[i]-memt[i] > 0 {
			rest[i] = k - mems[i] - memt[i]
		}
	}
	rests := make([]int, len(rest))
	idx := 0
	for k, _ := range rest {
		rests[idx] = k
		idx++
	...
	for _, a := range rests {
		for _, b := range rests {
			...
			if nows+a*9*pow10(mems[a]) > nowt+b*9*pow10(memt[b]) {
				if a == b {
					cases += rest[a] * (rest[b] - 1)
				} else {
					cases += rest[a] * rest[b]
				}
			}
	...
	total := (9*k - 8) * (9*k - 9)
	ans := float64(cases) / float64(total)
	out(ans)
}
