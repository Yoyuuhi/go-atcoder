...
func main() {
	...
	for i := 0; i < n; i++ {
		s, t, x := ni3()
		sts1[i] = []int{s - x, t - x, x}
		memst[s-x] = true
		memst[s-x-1] = true
		memst[t-x] = true
		memst[t-x+1] = true
	}
	sort.Slice(sts1, func(i, j int) bool {
		return sts1[i][2] > sts1[j][2]
	})

	sst := make([]int, len(memst))
	idx := 0
	for k := range memst {
		sst[idx] = k
		idx++
	...
	tr := newlazystree(len(mst), stmin, stset)
	for _, st := range sts1 {
		tr.rc(mst[st[0]], mst[st[1]], st[2])
	}

	for i := 0; i < q; i++ {
		d := ni()
		ci := bs(0, len(sst)-1, func(c int) bool {
			return sst[c] <= d
		})
		a := tr.query(ci, ci+1)
		if a == inf {
