...
func main() {
	...
	count := 0
	countCoins(a, b, c, x, 0, 0, 0, &count)
	out(count)
}

func countCoins(a, b, c, x, i, j, k int, count *int) {
	if 500*i+100*j+50*k == x {
		*count += 1
	}

	if c > k {
		countCoins(a, b, c, x, i, j, k+1, count)
	} else if c == k && b > j {
		countCoins(a, b, c, x, i, j+1, 0, count)
	} else if b == j && a > i {
		countCoins(a, b, c, x, i+1, 0, 0, count)
	}
}
