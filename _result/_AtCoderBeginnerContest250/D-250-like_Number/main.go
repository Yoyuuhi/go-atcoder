...
func main() {
	...
	ans := 0
	for p := 0; p < len(primes)-1; p++ {
		if primes[p]*primes[p+1]*primes[p+1]*primes[p+1] > n {
			break
		}
		for q := p + 1; q < len(primes); q++ {
			if primes[p]*primes[q]*primes[q]*primes[q] <= n {
				ans++
				continue
			}
			break
