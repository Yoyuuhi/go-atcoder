...
func main() {
	...
	n, q := ni2()
	memNumPos := make(map[int]int, n)
	memPosNum := make(map[int]int, n)
	for i := 1; i <= n; i++ {
		memNumPos[i] = i
		memPosNum[i] = i
	}
	...
	for i := 0; i < q; i++ {
		...
		nextNum := memPosNum[nextPos]

		memNumPos[x], memNumPos[nextNum] = memNumPos[nextNum], memNumPos[x]
		memPosNum[pos], memPosNum[nextPos] = memPosNum[nextPos], memPosNum[pos]
	}
