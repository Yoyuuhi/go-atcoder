...
func main() {
	...
	cumemb := newcusum2d(memb)
	cumemw := newcusum2d(memw)

	ans := 0
	for i := 1; i <= 2*k; i++ {
		for j := 1; j <= 2*k; j++ {
			if i+k-1 > 2*k || j+k-1 > 2*k {
				continue
			}
			a1 := 0
			a1 += cumemb.get(i-1, j-1, i+k-1, j+k-1)
			a1 += cumemb.get(0, 0, i-1, j-1)
			a1 += cumemb.get(0, j+k-1, i-1, 2*k)
			a1 += cumemb.get(i+k-1, 0, 2*k, j-1)
			a1 += cumemb.get(i+k-1, j+k-1, 2*k, 2*k)

			a2 := 0
			a2 += cumemw.get(0, 0, 2*k, 2*k)
			a2 -= cumemw.get(i-1, j-1, i+k-1, j+k-1)
			a2 -= cumemw.get(0, 0, i-1, j-1)
			a2 -= cumemw.get(0, j+k-1, i-1, 2*k)
			a2 -= cumemw.get(i+k-1, 0, 2*k, j-1)
			a2 -= cumemw.get(i+k-1, j+k-1, 2*k, 2*k)
			ans = max(ans, a1+a2)

			a1 = 0
			a1 += cumemw.get(i-1, j-1, i+k-1, j+k-1)
			a1 += cumemw.get(0, 0, i-1, j-1)
			a1 += cumemw.get(0, j+k-1, i-1, 2*k)
			a1 += cumemw.get(i+k-1, 0, 2*k, j-1)
			a1 += cumemw.get(i+k-1, j+k-1, 2*k, 2*k)

			a2 = 0
			a2 += cumemb.get(0, 0, 2*k, 2*k)
			a2 -= cumemb.get(i-1, j-1, i+k-1, j+k-1)
			a2 -= cumemb.get(0, 0, i-1, j-1)
			a2 -= cumemb.get(0, j+k-1, i-1, 2*k)
			a2 -= cumemb.get(i+k-1, 0, 2*k, j-1)
			a2 -= cumemb.get(i+k-1, j+k-1, 2*k, 2*k)
			ans = max(ans, a1+a2)
		}
	}
	out(ans)
...
func (c *cusum2d) get(x1, y1, x2, y2 int) int {
	if x2 >= len(c.s) || y2 >= len(c.s) || x1 >= len(c.s) || y1 >= len(c.s) {
		return 0
	}
