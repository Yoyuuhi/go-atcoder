...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		s--
		t--
		edges1[s] = append(edges1[s], edge{to: t})
	...
	for i := 0; i < n; i++ {
		if memMin[i] == 0 || as[i] < memMin[i] {
			memMin[i] = as[i]
		}
		for _, edge := range edges1[i] {
			to := edge.to
			if memMin[to] == 0 || memMin[i] < memMin[to] {
				memMin[to] = memMin[i]
			}
		}
	}
	for i := n - 1; i >= 0; i-- {
		if memMax[i] == 0 || as[i] > memMax[i] {
			memMax[i] = as[i]
		}
		for _, edge := range edges2[i] {
			to := edge.to
			if memMax[to] == 0 || memMax[i] > memMax[to] {
				memMax[to] = memMax[i]
			}
		}
	}
	...
	for i := 0; i < n; i++ {
		...
		for _, edge := range edges1[i] {
			to := edge.to
			ans = max(ans, memMax[to]-memMin[i])
		}
	}
	out(ans)
}
