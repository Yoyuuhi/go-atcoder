...
func main() {
	...
	var startI int
	for i := 0; i < n; i++ {
		if isConnected(circles[i], circle{
			x: sx,
			y: sy,
			r: 0,
		}) {
	...
	graphDfs(edges, startI, targetI, make(map[int]int, n))
	if ans {
		out("Yes")
	} else {
		out("No")
...
func isConnected(c1, c2 circle) bool {
	disPow := uint64(c1.x-c2.x)*uint64(c1.x-c2.x) + uint64(c1.y-c2.y)*uint64(c1.y-c2.y)
	if disPow > uint64(c1.r+c2.r)*uint64(c1.r+c2.r) {
		return false
	}
...
func graphDfs(edges [][]edge, now int, t int, passed map[int]int) {
	if now == t || ans == true {
		ans = true
		return
	}
	...
	for _, edge := range edges[now] {
		if passed[edge.to] != 1 {
			graphDfs(edges, edge.to, t, passed)
		}
	}
}
