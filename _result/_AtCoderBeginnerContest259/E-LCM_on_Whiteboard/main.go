...
func main() {
	...
	n := ni()
	memPETime := map[[2]int]int{}
	memPMaxe := map[int]int{}
	...
	for i := 0; i < n; i++ {
		m := ni()
		peS[i] = make([][]int, 2)
		peS[i][0], peS[i][1] = ni2s(m)

		for j := 0; j < m; j++ {
			memPETime[[2]int{peS[i][0][j], peS[i][1][j]}] += 1
			memPMaxe[peS[i][0][j]] = max(memPMaxe[peS[i][0][j]], peS[i][1][j])
		}
	}

	ans := 0
	for i := 0; i < n; i++ {
		for j := 0; j < len(peS[i][0]); j++ {
			if memPMaxe[peS[i][0][j]] == peS[i][1][j] && memPETime[[2]int{peS[i][0][j], peS[i][1][j]}] == 1 {
				ans++
				break
			}
		}
