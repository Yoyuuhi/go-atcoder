...
func main() {
	...
	passed := make(map[int]bool, 2*n)
	for i := 0; i < 2*n; i++ {
		passed[i] = false
	}
	simulate(edges, 0, 0, 0, passed)
	out(ans)
}

func simulate(edges [][]edge, now, tempAns, count int, passed map[int]bool) {
	if count == n {
		ans = max(ans, tempAns)
		return
	}

	if passed[now] || now == 2*n-1 {
		return
	}
	passed[now] = true
	...
	for _, edge := range edges[now] {
		if !passed[edge.to] {
			...
			simulate(edges, to, tempAns^edge.cost, count+1, passed)
			passed[edge.to] = false
		}
	}
	passed[now] = false
}
