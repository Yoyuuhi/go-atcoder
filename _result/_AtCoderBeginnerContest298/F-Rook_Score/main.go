...
func main() {
	...
	mrc := map[int]map[int]bool{}
	for i := 0; i < n; i++ {
		rcx[i] = nis(3)
		memr[rcx[i][0]] = memr[rcx[i][0]] + rcx[i][2]
		memc[rcx[i][1]] = memc[rcx[i][1]] + rcx[i][2]
		if _, e := mrc[rcx[i][0]]; !e {
			mrc[rcx[i][0]] = map[int]bool{}
		}
		mrc[rcx[i][0]][rcx[i][1]] = true
	}
	ans := 0
	for _, rx := range rcx {
		r, c, x := rx[0], rx[1], rx[2]
		ans = max(ans, memr[r]+memc[c]-x)
	}
	mcs := make([]int, 0, n)
	for k := range memc {
		mcs = append(mcs, k)
