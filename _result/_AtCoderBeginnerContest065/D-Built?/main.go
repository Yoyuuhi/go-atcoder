...
func main() {
	...
	edges := make([]edge, len(xs)+len(ys))
	ind := 0
	for i := 0; i < len(xs)-1; i++ {
		j := i + 1
		s, t, d := memX[xs[i]], memX[xs[j]], abs(xs[i]-xs[j])
		edges[ind] = edge{from: s, to: t, cost: d}
		ind++
	}
	for i := 0; i < len(ys)-1; i++ {
		j := i + 1
		s, t, d := memY[ys[i]], memY[ys[j]], abs(ys[i]-ys[j])
		edges[ind] = edge{from: s, to: t, cost: d}
		ind++
	}
	edges = edges[:ind]
	ans := editKruskal(edges, uf)

	out(ans)
}

func editKruskal(edges []edge, uf *unionFind) int {
