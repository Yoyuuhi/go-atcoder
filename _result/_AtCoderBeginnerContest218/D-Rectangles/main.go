...
func main() {
	...
	memXYs := map[int][]int{}
	for i := 0; i < n; i++ {
		if _, e := memXYs[ps[i].x]; !e {
			memXYs[ps[i].x] = []int{}
		}
		memXYs[ps[i].x] = append(memXYs[ps[i].x], ps[i].y)
	}

	for k, _ := range memXYs {
		sorti(memXYs[k])
	}

	memSL := make([]map[int]map[int]bool, len(memXYs))
	ind := 0
	for _, v := range memXYs {
		memSL[ind] = map[int]map[int]bool{}
		for i := 0; i < len(v)-1; i++ {
			for j := i + 1; j < len(v); j++ {
				if _, e := memSL[ind][v[i]]; !e {
					memSL[ind][v[i]] = map[int]bool{}
				}
				memSL[ind][v[i]][v[j]-v[i]] = true
			}
		}
		ind++
	}
	...
	ans := 0
	for i := 0; i < len(memSL)-1; i++ {
		for j := i + 1; j < len(memSL); j++ {
			for k1, v1 := range memSL[i] {
				for k2, _ := range v1 {
					if memSL[j][k1][k2] {
						ans++
					}
