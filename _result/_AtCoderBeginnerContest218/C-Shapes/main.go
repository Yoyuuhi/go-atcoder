...
searchO1:
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			if string(sX[i][j]) == "#" {
				...
				y := j
				if !(o1.x == x && o1.y == y) {
					if _, e := memRel1[x-o1.x]; !e {
						memRel1[x-o1.x] = map[int]bool{}
					}
					memRel1[x-o1.x][y-o1.y] = true
					count1 += 1
				}
			}
		}
	...
	ans := false
	ans = ans || search1(n, count1, memRel1, tX)
	ans = ans || search2(n, count1, memRel1, tX)
	ans = ans || search3(n, count1, memRel1, tX)
	ans = ans || search4(n, count1, memRel1, tX)
...
func search1(n int, count1 int, memRel1 map[int]map[int]bool, tX []string) bool {
	var o point
searchO:
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
...
func search2(n int, count1 int, memRel1 map[int]map[int]bool, tX []string) bool {
	var o point
searchO:
	for j := n - 1; j >= 0; j-- {
		for i := 0; i < n; i++ {
...
func search3(n int, count1 int, memRel1 map[int]map[int]bool, tX []string) bool {
	var o point
searchO:
	for i := n - 1; i >= 0; i-- {
		for j := n - 1; j >= 0; j-- {
...
func search4(n int, count1 int, memRel1 map[int]map[int]bool, tX []string) bool {
	var o point
searchO:
	for j := 0; j < n; j++ {
		for i := n - 1; i >= 0; i-- {
