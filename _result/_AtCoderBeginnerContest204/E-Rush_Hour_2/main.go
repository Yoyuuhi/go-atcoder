...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, cost: c, d: d, d1: sqrt(d)})
		edges[t] = append(edges[t], edge{to: s, cost: c, d: d, d1: sqrt(d)})
	}
...
func (g *graph) editDijkstra(start int) []int {
	...
	for !que.IsEmpty() {
		...
		for _, edge := range g.edges[st.node] {
			...
			d1 := edge.d1
			if d1 < st.score {
				newScore = st.score + edge.cost + d/(st.score+1)
				newScore1 := st.score + 1 + edge.cost + d/(st.score+2)
				newScore = min(newScore1, newScore)
			} else {
				newScore1 := d1 + edge.cost + d/(d1+1)
				newScore3 := d1 + 1 + edge.cost + d/(d1+2)
				newScore = min(newScore1, newScore3)
				if d1 != 0 {
					newScore2 := d1 - 1 + edge.cost + d/d1
					newScore = min(newScore2, newScore)
				}
			}
