...
func main() {
	...
	edges := make([]map[int]bool, len(mem)+1)
	...
	seen = make([]int, len(mem)+1)
	d = make([]int, len(mem)+1)
	dfs(edges, 0)
	if !circle {
		out(d[0])
	} else {
...
func dfs(edges []map[int]bool, now int) int {
	if circle {
		return 0
	}
	...
	seen[now] = 1
	for e := range edges[now] {
		if seen[e] == 1 {
			circle = true
			return 0
		}
		d[now] = max(d[now], dfs(edges, e)+1)
	}
	seen[now] = 2
	return d[now]
}
