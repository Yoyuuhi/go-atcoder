	...
	for i := a + b + c; i < 298; i++ {
		for j := 0; j < 100; j++ {
			for k := 0; k < 100; k++ {
				...
				if j == 99 {
					ans += dp[i][j][k] * (fi - fabc + 1) * fj / fi
				} else {
					dp[i+1][j+1][k] += dp[i][j][k] * fj / fi
				}
				if k == 99 {
					ans += dp[i][j][k] * (fi - fabc + 1) * fk / fi
				} else {
					dp[i+1][j][k+1] += dp[i][j][k] * fk / fi
				}
				if l == 99 {
					ans += dp[i][j][k] * (fi - fabc + 1) * fl / fi
				} else {
					dp[i+1][j][k] += dp[i][j][k] * fl / fi
				}
			}
		}
	}
