...
func main() {
	...
	for i := 0; i < h; i++ {
		...
		for j := 0; j < w; j++ {
			...
			if c == "S" {
				...
				memTs[c] = append(memTs[c], point{i, j})
				mem[i][j] = inf
			} else {
				if c == "G" {
					g = point{i, j}
	...
	que := newpq([]compFunc{
		func(p, q interface{}) int {
			if p.(pstate).score < q.(pstate).score {
				return -1
			} else if p.(pstate).score == q.(pstate).score {
				return 0
			}
			return 1
		},
	})
	heap.Push(que, pstate{0, s.x, s.y})

	isUpdated := map[string]bool{}
	for !que.IsEmpty() {
		now := heap.Pop(que).(pstate)
		x := now.x
		y := now.y
		if now.score > mem[x][y] {
			continue
		}
		if x == g.x && y == g.y {
			out(now.score)
		...
		c := string(ax[x][y])
		if c >= "a" && c <= "z" && !isUpdated[c] {
			for _, p := range memTs[c] {
				if mem[p.x][p.y] > mem[x][y]+1 {
					mem[p.x][p.y] = mem[x][y] + 1
					heap.Push(que, pstate{mem[p.x][p.y], p.x, p.y})
				}
			}
			isUpdated[c] = true
		}
		...
		for _, v := range vectorsA {
			...
			if c != "#" {
				if mem[i][j] > mem[x][y]+1 {
					mem[i][j] = mem[x][y] + 1
					heap.Push(que, pstate{mem[i][j], i, j})
				}
			}
		}
	}
