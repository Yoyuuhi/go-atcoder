...
func isApplicable(ns []int, temp, m, k int) bool {
	diffNs := make([]int, len(ns))
	...
	for i, num := range ns {
		if temp >= num {
			...
			tempUpper := temp
			for rightShiftNum > 0 {
				numBit = append(numBit, rightShiftNum&1)
				rightShiftNum = rightShiftNum >> 1
			}
			for i := len(numBit) - 1; i >= 0; i-- {
				if 1<<i&tempUpper == 0 && tempUpper|(1<<i-1) < num {
					tempUpper += 1 << i
				}
			}
			diffNs[i] = tempUpper - num
		}
	}
	sorti(diffNs)
	sum := 0
