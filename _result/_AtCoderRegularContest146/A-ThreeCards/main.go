...
func main() {
	...
	resultSlice := make([]int, 3)
	for order := 0; order < 3; order++ {
		resultSlice[order] = ns[len(ns)-1]
		ns = ns[:len(ns)-1]
	}
	var result string
	if len(fmt.Sprint(resultSlice[0])) == len(fmt.Sprint(resultSlice[1])) && len(fmt.Sprint(resultSlice[0])) == len(fmt.Sprint(resultSlice[2])) {
		result = fmt.Sprint(resultSlice[0]) + fmt.Sprint(resultSlice[1]) + fmt.Sprint(resultSlice[2])
	} else {
		var max int
		var tempNum string
		for firstI := 0; firstI < 3; firstI++ {
			tempNum = fmt.Sprint(resultSlice[firstI])
			for secondI := 0; secondI < 3; secondI++ {
				var tempNum1 string
				if firstI != secondI {
					tempNum1 = tempNum + fmt.Sprint(resultSlice[secondI])
				} else {
					continue
				}
				var tempNum2 string
				for thirdI := 0; thirdI < 3; thirdI++ {
					if thirdI != firstI && thirdI != secondI {
						tempNum2 = tempNum1 + fmt.Sprint(resultSlice[thirdI])
					} else {
						continue
					}
					i, _ := strconv.Atoi(tempNum2)
					if i > max {
						max = i
					}
				}
