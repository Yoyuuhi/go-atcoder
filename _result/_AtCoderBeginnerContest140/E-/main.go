...
func main() {
	...
	for i, p := range ps {
		...
		var a, b int
		if i > 0 {
			l1 = bs(0, i+1, func(c int) bool {
				return s.query(c, i+1) > p
			})
			if l1 != -1 && l1 > 0 {
				l2 = bs(0, l1, func(c int) bool {
					return s.query(c, l1) > p
				})
			}
		}
		if i+1 < n {
			r1 = bs(n, i, func(c int) bool {
				return s.query(i, c) > p
			})
			if r1 != -1 {
				r2 = bs(n, r1, func(c int) bool {
					return s.query(r1, c) > p
				})
			}
		}
