...
func main() {
	...
	q := ni()
	heapMin := newIntHeap(asc)
	heapMax := newIntHeap(desc)
	memExist := make(map[int]int, q)
	...
	for i := 0; i < q; i++ {
		...
		case 2:
			x, c := ni2()
			memExist[x] -= min(memExist[x], c)
		case 3:
			for memExist[heapMin.GetRoot()] == 0 {
				heapMin.Pop()
			}
			for memExist[heapMax.GetRoot()] == 0 {
				heapMax.Pop()
			}
			existOut = true
			out(heapMax.GetRoot() - heapMin.GetRoot())
		}
	}
	if !existOut {
		out()
