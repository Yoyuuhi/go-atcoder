...
func main() {
	...
	for a, m := range mem {
		for b := range m {
			...
			if a != 0 && b != 0 {
				for i := 1; i <= mem[a][b]; i++ {
					t = madd(t, mmul(temp, cf.combination(mem[a][b], i)))
				}
				mem[a][b] = 0
				if _, e := mem[-b]; e && a > 0 {
					for i := 1; i <= mem[-b][a]; i++ {
						t = madd(t, mmul(temp, cf.combination(mem[-b][a], i)))
					}
					mem[-b][a] = 0
				}
				if _, e := mem[b]; e && a < 0 {
			...
			if mem[0][1] == 0 && mem[1][0] == 0 {
				continue
			}

			if _, e := mem[0]; e {
				for i := 1; i <= mem[0][1]; i++ {
					t = madd(t, mmul(temp, cf.combination(mem[0][1], i)))
				}
				mem[0][1] = 0
			}
			if _, e := mem[1]; e {
				for i := 1; i <= mem[1][0]; i++ {
					t = madd(t, mmul(temp, cf.combination(mem[1][0], i)))
				}
				mem[1][0] = 0
			}
			t = madd(t, temp)
