...
func simulate(nNow, sum int) {
	if mem[sum] {
		return
	}
	mem[sum] = true

	if ans || x-sum < sumMin[len(sumMin)-1]-sumMin[nNow] || x-sum > sumMax[len(sumMax)-1]-sumMax[nNow] {
		return
	}

	if (sum == x && nNow == n) || x-sum == sumMin[len(sumMin)-1]-sumMin[nNow] || x-sum == sumMax[len(sumMax)-1]-sumMax[nNow] {
		ans = true
		return
	}
	if nNow+1 < len(sumMin)-1 {
		simulate(nNow+1, sum+aS[nNow])
		simulate(nNow+1, sum+bS[nNow])
	}
}
