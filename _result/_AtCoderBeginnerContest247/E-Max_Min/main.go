...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		if i > 0 {
			memXY[i][0] += memXY[i-1][0]
			memXY[i][1] += memXY[i-1][1]
		}
	}

	if memXY[n-1][0] == 0 || memXY[n-1][1] == 0 {
		out(0)
		return
	}

	l := 0
	for l < n && (as[l] > x || as[l] < y) {
		l++
	}
	r := l
	rr := r
...
func findR(l, r int, n int, memXY [][]int, as []int) (bool, int) {
	for {
		if r == n {
			return true, r
		}
		...
		xNum := memXY[r][0] - memXY[l][0]
		yNum := memXY[r][1] - memXY[l][1]
...
func findRR(rr int, n int, as []int) int {
	for rr < n && as[rr] <= x && as[rr] >= y {
		rr++
	}
	return rr - 1
}
