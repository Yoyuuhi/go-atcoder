...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		case 1:
			x, c := ni2()
			vars = append(vars, x)
			num = append(num, c+num[len(num)-1])
			sum = append(sum, c*x+sum[len(sum)-1])
		case 2:
			c := ni()
			leftNum := bs(len(num)-1, 0, func(index int) bool {
				return num[index] >= left
			})
			rightNum := bs(len(num)-1, 0, func(index int) bool {
				return num[index] >= left+c
			})
			ans := 0
			ans += (num[leftNum] - left) * vars[leftNum]
			ans += sum[rightNum-1] - sum[leftNum]
			ans += (left + c - num[rightNum-1]) * vars[rightNum]
