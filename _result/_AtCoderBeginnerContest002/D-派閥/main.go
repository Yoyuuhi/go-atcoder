...
func main() {
	...
	memKnow := map[int]map[int]bool{}
	for i := 0; i < m; i++ {
		x, y := ni2()
		if _, e := memKnow[x]; !e {
			memKnow[x] = map[int]bool{}
		}
		if _, e := memKnow[y]; !e {
			memKnow[y] = map[int]bool{}
		}
		memKnow[x][y] = true
		memKnow[y][x] = true
	}
	...
	for bits := 1; bits < (1 << n); bits++ {
		...
		if knowEachOther(bitMap, memKnow) {
			ans = max(ans, len(bitMap))
		}
	}
	out(ans)
}

func knowEachOther(bitMap map[int]bool, memKnow map[int]map[int]bool) bool {
	bitAry := make([]int, len(bitMap))
	ind := 0
	...
	for i := 0; i < len(bitAry)-1; i++ {
		for j := i + 1; j < len(bitAry); j++ {
			if _, e := memKnow[bitAry[i]]; !e || !memKnow[bitAry[i]][bitAry[j]] {
				return false
			}
		}
	}
