...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		edges[b] = append(edges[b], edge{to: a})
		uf.unite(a, b)
	}

	memGroups := map[int]bool{}
	for i := 0; i < n; i++ {
		memGroups[uf.root(i)] = true
	}

	memGroupNum = map[int]int{}
	for g, _ := range memGroups {
		memGroupNum[g] = 0
		passed := map[int]bool{}
		graphDfs(edges, g, passed)
		passedS := make([]int, len(passed))
		ind := 0
		for k, _ := range passed {
			passedS[ind] = k
			ind++
		}
		color(edges, map[int]int{}, passedS, 0, g)
	}
	ans := 1
	for _, v := range memGroupNum {
		ans *= v
	}
	out(ans)
}

func graphDfs(edges [][]edge, now int, passed map[int]bool) {
	passed[now] = true
	for _, e := range edges[now] {
		if !passed[e.to] {
			graphDfs(edges, e.to, passed)
		}
	}
}

func color(edges [][]edge, colored map[int]int, passedS []int, ind int, g int) {
color:
	for i := 1; i < 4; i++ {
		for _, edge := range edges[passedS[ind]] {
			if colored[edge.to] == i {
				continue color
			}
		}
		if ind == len(passedS)-1 {
			memGroupNum[g] += 1
			continue
		}
		colored[passedS[ind]] = i
		color(edges, colored, passedS, ind+1, g)
		colored[passedS[ind]] = 0
	}
}
