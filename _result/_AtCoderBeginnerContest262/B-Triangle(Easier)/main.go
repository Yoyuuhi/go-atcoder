...
func main() {
	...
	memSearched := make(map[int]int, n)

	ans := 0
	for i := 0; i < n; i++ {
		for j := 0; j < len(edges[i])-1; j++ {
			if memSearched[edges[i][j].to] == 1 {
				continue
			}
			for k := j + 1; k < len(edges[i]); k++ {
				if memSearched[edges[i][k].to] == 1 {
					continue
				}
				if isTriangle(edges[i][j], edges[i][k], edges) {
					ans += 1
				}
			}
		}
...
func isTriangle(a, b edge, edges [][]edge) bool {
	for i := 0; i < len(edges[a.to]); i++ {
		if edges[a.to][i].to == b.to {
			return true
		}
	}
	return false
