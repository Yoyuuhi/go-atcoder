...
func main() {
	...
	memDistanceNum := make(map[int]int, n)
	for i := 0; i < n; i++ {
		if i >= ps[i] {
			memDistanceNum[i-ps[i]] += 1
		} else {
			memDistanceNum[i-ps[i]+n] += 1
		}
	}

	var dises []int
	for i := 0; i < n; i++ {
		dises = append(dises, i)
	}

	ans := 0
	for wStart := 0; wStart < len(dises); wStart++ {
		sum := memDistanceNum[dises[wStart]]
		if (wStart+1)%len(dises) != wStart {
			sum += memDistanceNum[dises[(wStart+1)%len(dises)]]
		}
		if (wStart+2)%len(dises) != wStart && (wStart+2)%len(dises) != (wStart+1)%len(dises) {
			sum += memDistanceNum[dises[(wStart+2)%len(dises)]]
		}
		ans = max(ans, sum)
	}
	out(ans)
