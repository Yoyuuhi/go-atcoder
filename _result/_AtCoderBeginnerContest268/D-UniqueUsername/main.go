...
func main() {
	...
	simulate(mem, strS1, unders, "", ans)
	if *ans != true {
		out(-1)
	}
}

func simulate(mem map[string]int, restStrs []string, restUnders int, calcStr string, finished *bool) {
	if *finished == true {
		return
	}
	...
	for i := 0; i < len(restStrs); i++ {
		...
		newRestStrs = append([]string{}, restStrs[:i]...)
		if i+1 != len(restStrs) {
			newRestStrs = append(newRestStrs, restStrs[i+1:]...)
		}

		if len(newRestStrs) != 0 {
			for j := 1; j <= restUnders; j++ {
				newCalcStrJ := newCalcStr
				for k := 0; k < j; k++ {
					newCalcStrJ += "_"
				}
				simulate(mem, newRestStrs, restUnders-j, newCalcStrJ, finished)
			}
		} else {
			simulate(mem, newRestStrs, restUnders, newCalcStr, finished)
		}
	}
}
