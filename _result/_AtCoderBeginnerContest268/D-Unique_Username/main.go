...
func main() {
	...
	unders := 16 - totalLength - (n - 1)

	mem := make(map[string]bool, m)
	for i := 0; i < m; i++ {
		str := ns()
		mem[str] = true
	}

	simulate(mem, strS1, unders, "")
	if !finished {
		out(-1)
	}
}

func simulate(mem map[string]bool, restStrs []string, restUnders int, calcStr string) {
	if finished == true {
		return
	}

	if len(restStrs) == 0 {
		if !mem[calcStr] && len(calcStr) >= 3 && len(calcStr) <= 16 {
			out(calcStr)
			finished = true
			return
		}
	...
	for i := 0; i < len(restStrs); i++ {
		newCalcStr := calcStr
		newCalcStr += restStrs[i]

		var newRestStrs []string
		for j := 0; j < len(restStrs); j++ {
			if i != j {
				newRestStrs = append(newRestStrs, restStrs[j])
			}
		}

		if len(newRestStrs) != 0 {
			newCalcStr += "_"

			for j := 0; j <= restUnders; j++ {
				newCalcStrJ := newCalcStr
				for k := 0; k < j; k++ {
					newCalcStrJ += "_"
				}
				simulate(mem, newRestStrs, restUnders-j, newCalcStrJ)
			}
		} else {
			simulate(mem, newRestStrs, restUnders, newCalcStr)
		}
	}
}
