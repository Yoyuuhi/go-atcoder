...
func main() {
	...
	zeroIs := []int{}
	if string(s[0]) == "<" {
		zeroIs = append(zeroIs, 0)
	}
	if string(s[n-1]) == ">" {
		zeroIs = append(zeroIs, n)
	}

	for i := 1; i < n; i++ {
		if string(s[i-1]) == ">" && string(s[i]) == "<" {
			zeroIs = append(zeroIs, i)
		}
	}

	for _, i := range zeroIs {
		calc(mem, n, i, 0, 1, s)
		calc(mem, n, i, 0, -1, s)
	}
...
func calc(mem map[int]int, n, nowI, ans, dir int, s string) {
	mem[nowI] = max(mem[nowI], ans)

	switch dir {
	case 1:
		if nowI < n && string(s[nowI]) == "<" {
			calc(mem, n, nowI+dir, ans+1, dir, s)
		}
	case -1:
		if nowI-1 >= 0 && string(s[nowI-1]) == ">" {
			calc(mem, n, nowI+dir, ans+1, dir, s)
		}
	}
}
