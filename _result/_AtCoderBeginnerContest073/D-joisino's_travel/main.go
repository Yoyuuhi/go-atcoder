...
func main() {
	...
	score, _ := graph.floydWarshall()

	dp := make([]map[int]map[int]int, R+1)
	for i := range dp {
		dp[i] = map[int]map[int]int{}
	}
	for i := 0; i < R; i++ {
		dp[1][1<<i] = map[int]int{}
		dp[1][1<<i][i] = 0
	}
	for i := 1; i < R; i++ {
		for bit, ma := range dp[i] {
			for j := 0; j < R; j++ {
				if hasbit(bit, j) {
					continue
				}
				bit1 := bit | (1 << j)
				if _, e := dp[i+1][bit1]; !e {
					dp[i+1][bit1] = map[int]int{}
					dp[i+1][bit1][j] = inf
				}
				for k, v := range ma {
					if dp[i+1][bit1][j] == 0 {
					...
					dp[i+1][bit1][j] = min(dp[i+1][bit1][j], v+score[rs[k]][rs[j]])
				}
			}
		}
	}
