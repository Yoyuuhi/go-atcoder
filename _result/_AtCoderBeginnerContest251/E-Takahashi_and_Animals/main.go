...
func main() {
	...
	for i := 2; i < n; i++ {
		for n, c := range dp1[i] {
			...
			if n == i {
				dp1[i+1][n] = min(dp1[i+1][n], c)
				dp1[i+1][n+1] = min(dp1[i+1][n+1], c+as[i-1])
			} else if n+1 == i {
				dp1[i+1][n+2] = min(dp1[i+1][n+2], c+as[i-1])
			}
		}
	}
	ans1 := dp1[n][n]

	dp2 := make([]map[int]int, n+1)
	for i := 0; i < n+1; i++ {
		dp2[i] = map[int]int{}
		dp2[i][i] = inf
		dp2[i][i-1] = inf
	...
	for i := 1; i < n; i++ {
		for n, c := range dp2[i] {
			...
			if n == i {
				dp2[i+1][n] = min(dp2[i+1][n], c)
				dp2[i+1][n+1] = min(dp2[i+1][n+1], c+as[i-1])
			} else if n+1 == i {
				dp2[i+1][n+2] = min(dp2[i+1][n+2], c+as[i-1])
			}
		}
	}
	ans2 := min(dp2[n][n], dp2[n][n-1])
	out(min(ans1, ans2))
}
