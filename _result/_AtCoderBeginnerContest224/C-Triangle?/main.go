...
func main() {
	...
	ans := 0
	gcdMap := map[twoPoint]int{}
	for i := 0; i < n-2; i++ {
		for j := i + 1; j < n-1; j++ {
			for k := j + 1; k < n; k++ {
				if checkTri(ps[i], ps[j], ps[k], gcdMap) {
					ans++
				}
			}
		}
...
func checkTri(p1, p2, p3 point, gcdMap map[twoPoint]int) bool {
	var g1, g2 int
	if v, e := gcdMap[twoPoint{p1: p1, p2: p2}]; e {
		g1 = v
	} else {
		g1 = gcd(abs(p1.x-p2.x), abs(p1.y-p2.y))
		gcdMap[twoPoint{p1: p1, p2: p2}] = g1
	}
	v1 := point{x: (p1.x - p2.x) / g1, y: (p1.y - p2.y) / g1}

	if v, e := gcdMap[twoPoint{p1: p1, p2: p3}]; e {
		g2 = v
	} else {
		g2 = gcd(abs(p1.x-p3.x), abs(p1.y-p3.y))
		gcdMap[twoPoint{p1: p1, p2: p3}] = g2
	}
	v2 := point{x: (p1.x - p3.x) / g2, y: (p1.y - p3.y) / g2}
