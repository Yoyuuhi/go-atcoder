...
func main() {
	...
	memA := map[int][]infoMem{}
	for _, m := range mem {
		memA[m.a] = append(memA[m.a], m)
	}
	as := make([]int, len(memA))
	idx := 0
	for a := range memA {
		as[idx] = a
		idx++
	...
	for i := 1; i < len(as); i++ {
		for _, m := range memA[as[i]] {
			idx := m.i
			p := m.p

			if v, e := maxR[p.x]; e {
				dp[idx] = max(dp[idx], v+1)
			}
			if v, e := maxC[p.y]; e {
				dp[idx] = max(dp[idx], v+1)
			}
		...
		for _, m := range memA[as[i]] {
			idx := m.i
			p := m.p
			maxR[p.x] = max(maxR[p.x], dp[idx])
			maxC[p.y] = max(maxC[p.y], dp[idx])
		}
	}
	for _, ans := range dp {
		out(ans)
