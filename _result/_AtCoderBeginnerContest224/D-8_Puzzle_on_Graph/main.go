...
func main() {
	...
	memOrder := map[int]int{}
	for i := 1; i <= 8; i++ {
		memOrder[ni()-1] = i
	}

	if checkFinish(memOrder) {
		out(0)
		return
	}

	strS := convToStr(memOrder)

	ansMap := map[string]int{}
	ansMap[strS] = 0

	strList := list.New()
	strList.PushBack(strS)

	e := strList.Front()
	for e != nil {
		eMap := convToMap(e.Value.(string))
		...
		for _, edge := range edges[s] {
			eMap[s], eMap[edge.to] = eMap[edge.to], eMap[s]
			eStr := convToStr(eMap)
			if _, exist := ansMap[eStr]; !exist {
				ansMap[eStr] = ansMap[e.Value.(string)] + 1
				strList.PushBack(eStr)
			}
			eMap[s], eMap[edge.to] = eMap[edge.to], eMap[s]
		}
		e = e.Next()
	...
	if ansMap["123456780"] == 0 {
		out(-1)
	} else {
		out(ansMap["123456780"])
	}
}

func convToStr(mO map[int]int) string {
	tempMem := ""
	for j := 0; j < 9; j++ {
		tempMem += itoa(mO[j])
	}
...
func convToMap(str string) map[int]int {
	ans := map[int]int{}
	for i := 0; i < 9; i++ {
		ans[i] = atoi(string(str[i]))
	}
	return ans
}

func checkFinish(mem map[int]int) bool {
	for i := 0; i < 8; i++ {
		if mem[i] != i+1 {
			return false
		}
