...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		case 3:
			p := ni()
			temp[0] = [3]int{-1, 0, 2 * p}
			temp[1] = [3]int{0, 1, 0}
		case 4:
			p := ni()
			temp[0] = [3]int{1, 0, 0}
			temp[1] = [3]int{0, -1, 2 * p}
		}
		temp[2] = [3]int{0, 0, 1}

		if i > 0 {
			temp = calc(mem[i-1], temp)
		}
		mem[i] = temp
	}
	...
	for i := 0; i < q; i++ {
		...
		x, y := xys[b][0], xys[b][1]
		x1 := mem[a][0][0]*x + mem[a][0][1]*y + mem[a][0][2]
		y1 := mem[a][1][0]*x + mem[a][1][1]*y + mem[a][1][2]
		out(x1, y1)
	}
}

func calc(m2, m1 [3][3]int) [3][3]int {
	ans := [3][3]int{}
	for i := 0; i < 3; i++ {
		for j := 0; j < 3; j++ {
			ans[i][j] = m1[i][0]*m2[0][j] + m1[i][1]*m2[1][j] + m1[i][2]*m2[2][j]
		}
	}
	return ans
}
