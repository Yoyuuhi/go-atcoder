...
func main() {
	...
	memIndexNum := map[int]int{}
	memNumTimes := map[int]int{}
	memNumBigger := map[int]int{}
	memBiggerNum := map[int]int{}
	ansCount := map[int]int{}
	...
	keys := make([]int, len(memNumTimes))
	i := 0
	for k, _ := range memNumTimes {
		keys[i] = k
		i++
	}
	sortir(keys)
	...
	for _, key := range keys {
		memNumBigger[key] = sum
		memBiggerNum[sum] = key
		ansCount[sum] += 1
		sum += 1
	}
	// out(memNumBigger)
	...
	for i := 0; i < n; i++ {
		out(memNumTimes[memBiggerNum[i]])
	}
}
