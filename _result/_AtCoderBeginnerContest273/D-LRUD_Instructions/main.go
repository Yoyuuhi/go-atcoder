...
func main() {
	...
	for i := 0; i < n; i++ {
		mapRC[rS[i]] = append(mapRC[rS[i]], cS[i])
		mapCR[cS[i]] = append(mapCR[cS[i]], rS[i])
	}
	...
	for i := 0; i < q; i++ {
		...
		case "L":
			to = max(1, cs-l)
			if _, e := mapRC[rs]; e {
				next := bs(0, len(mapRC[rs])-1, func(c int) bool {
					return mapRC[rs][c] < cs
				})
				if next != -1 && mapRC[rs][next] >= to {
					to = mapRC[rs][next] + 1
				}
			}
			cs = to
		case "R":
			to = min(w, cs+l)
			if _, e := mapRC[rs]; e {
				next := bs(len(mapRC[rs])-1, 0, func(c int) bool {
					return mapRC[rs][c] > cs
				})
				if next != -1 && mapRC[rs][next] <= to {
					to = mapRC[rs][next] - 1
		...
		case "U":
			to = max(1, rs-l)
			if _, e := mapCR[cs]; e {
				next := bs(0, len(mapCR[cs])-1, func(c int) bool {
					return mapCR[cs][c] < rs
				})
				if next != -1 && mapCR[cs][next] >= to {
					to = mapCR[cs][next] + 1
				}
			}
			rs = to
		case "D":
			to = min(h, rs+l)
			if _, e := mapCR[cs]; e {
				next := bs(len(mapCR[cs])-1, 0, func(c int) bool {
					return mapCR[cs][c] > rs
				})
				if next != -1 && mapCR[cs][next] <= to {
					to = mapCR[cs][next] - 1
