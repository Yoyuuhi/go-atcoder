...
func main() {
	...
	as := make([][]int, n)
	for i := 0; i < n; i++ {
		as[i] = []int{ni(), 2 * ni(), 2 * ni()}
	}

	memPlus := map[int]int{}
	memMinus := map[int]int{}
	memPs := map[int]bool{}
	for _, a := range as {
		switch a[0] {
		case 1:
			memPlus[a[1]] += 1
			memMinus[a[2]+1] += 1
			memPs[a[1]], memPs[a[2]+1] = true, true
		case 2:
			memPlus[a[1]] += 1
			memMinus[a[2]] += 1
			memPs[a[1]], memPs[a[2]] = true, true
		case 3:
			memPlus[a[1]+1] += 1
			memMinus[a[2]+1] += 1
			memPs[a[1]+1], memPs[a[2]+1] = true, true
		case 4:
			memPlus[a[1]+1] += 1
			memMinus[a[2]] += 1
			memPs[a[1]+1], memPs[a[2]] = true, true
		}
	}

	ps := make([]int, len(memPs))
	idx := 0
	for p, _ := range memPs {
		ps[idx] = p
		idx++
