...
func main() {
	...
	diffT := make([][]float64, n)

	idx := 0
	for _, s := range S {
		if float64(s[0]) != wsx || float64(s[1]) != wsy {
			diffS[idx] = []float64{float64(s[0]) - wsx, float64(s[1]) - wsy}
			idx++
		}
	}
	diffS = diffS[:idx]
	...
	idx = 0
	for _, s := range T {
		if float64(s[0]) != wtx || float64(s[1]) != wty {
			diffT[idx] = []float64{float64(s[0]) - wtx, float64(s[1]) - wty}
			idx++
		}
	}
	diffT = diffT[:idx]
	...
	for i := 0; i < len(diffS); i++ {
		j := (i + 1) % len(diffS)
		c1, c2 := diffS[i][0]*diffS[j][0]+diffS[i][1]*diffS[j][1], diffS[i][0]*diffS[j][1]-diffS[i][1]*diffS[j][0]

		l1, l2 := diffS[i][0]*diffS[i][0]+diffS[i][1]*diffS[i][1], diffS[j][0]*diffS[j][0]+diffS[j][1]*diffS[j][1]

		poss := false
		for r := 0; r < len(diffT); r++ {
			l3 := diffT[r][0]*diffT[r][0] + diffT[r][1]*diffT[r][1]
			if cnum(l1) != cnum(l3) {
				continue
			}
			for s := 0; s < len(diffT); s++ {
				l4 := diffT[s][0]*diffT[s][0] + diffT[s][1]*diffT[s][1]
				if cnum(l2) != cnum(l4) {
					continue
				}

				c3, c4 := diffT[r][0]*diffT[s][0]+diffT[r][1]*diffT[s][1], diffT[r][0]*diffT[s][1]-diffT[r][1]*diffT[s][0]
				if cnum(c1) == cnum(c3) && cnum(c2) == cnum(c4) {
					poss = true
					break
				}
			}
...
func calcW(s [][]int) (float64, float64) {
	sumx := float64(0)
	sumy := float64(0)
	for _, p := range s {
		sumx += float64(p[0])
	...
	return sumx / float64(len(s)), sumy / float64(len(s))
}

func cnum(n float64) float64 {
