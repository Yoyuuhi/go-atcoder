...
func main() {
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			...
			mem[x][y][0] = madd(mem[x][y][0], mem[x-1][y][1])
			mem[x][y][0] = madd(mem[x][y][0], mem[x][y-1][2])
			mem[x][y][0] = madd(mem[x][y][0], mem[x-1][y-1][3])

			mem[x][y][1] = madd(mem[x-1][y][1], mem[x][y][0])
			mem[x][y][2] = madd(mem[x][y-1][2], mem[x][y][0])
			mem[x][y][3] = madd(mem[x-1][y-1][3], mem[x][y][0])
		}
	}
	out(mem[h][w][0])
}
