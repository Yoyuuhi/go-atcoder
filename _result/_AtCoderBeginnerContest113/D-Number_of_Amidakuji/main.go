...
func main() {
	...
	for i := 0; i < h; i++ {
		for bit := 0; bit < 1<<(w-1); bit++ {
			...
			for now := 0; now < w; now++ {
				dp[i+1][next(now, bit)] = madd(dp[i+1][next(now, bit)], dp[i][now])
			}
		}
	}
	out(dp[h][k-1])
...
func poss(bit int, w int) bool {
	for i := 0; i < w-1; i++ {
		if hasbit(bit, i) && hasbit(bit, i+1) {
			return false
		}
	}
	return true
...
func next(now int, bit int) int {
	if now-1 >= 0 && hasbit(bit, now-1) {
		return now - 1
	}
	if hasbit(bit, now) {
		return now + 1
