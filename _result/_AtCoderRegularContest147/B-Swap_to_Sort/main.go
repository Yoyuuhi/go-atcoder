...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n-2; j++ {
			if ps[j]%2 != ps[j+2]%2 && ps[j]%2 != j%2 {
				calc("B", j)
			}
		}
	}
	for i := 0; i < n-1; i++ {
		if ps[i]%2 != ps[i+1] && ps[i]%2 == i%2 {
			calc("A", i)
		}
	}
	for i := 0; i < n; i++ {
...
func calc(c string, i int) {
	a := "A"
	ans.PushBack([]string{c, itoa(i + 1)})
	ps[i], ps[i+1+int(c[0]-a[0])] = ps[i+1+int(c[0]-a[0])], ps[i]
}
