...
func main() {
	...
	for i, ab := range abs {
		...
		for j := 0; j < t; j++ {
			...
			if j+a < t {
				if dp[i][j][0]+max(dp[i][j][1], res)+b > dp[i+1][j+a][0]+max(dp[i+1][j+a][1], res) {
					dp[i+1][j+a] = []int{dp[i][j][0] + b, dp[i][j][1]}
					ans = max(ans, dp[i][j][0]+max(dp[i][j][1], res)+b)
				}
			}
			if dp[i][j][0]+max(dp[i][j][1], maxRes[i]) > dp[i+1][j][0]+max(dp[i+1][j][1], res) {
				dp[i+1][j] = []int{dp[i][j][0], max(dp[i][j][1], b)}
				ans = max(ans, dp[i][j][0]+max(dp[i][j][1], maxRes[i]))
			}
		}
	}
