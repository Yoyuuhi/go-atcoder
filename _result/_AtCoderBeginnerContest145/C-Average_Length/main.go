...
func main() {
	...
	out(float64(length / float64(count)))
}

func simulate(ps []point, now int, tempAns float64, count int, passed map[int]bool) {
	if count == len(ps) {
		length += tempAns
		return
	}
	...
	for i, _ := range ps {
		...
		simulate(ps, i, tempAns+math.Sqrt(float64((ps[i].x-ps[now].x)*(ps[i].x-ps[now].x)+(ps[i].y-ps[now].y)*(ps[i].y-ps[now].y))), count+1, passed)
	}
	passed[now] = false
}
