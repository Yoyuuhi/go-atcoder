...
func main() {
	...
	for i := 1; i < n-1; i++ {
		dp[i+1][0] = max(dp[i+1][0], dp[i][0]+as[i])
		dp[i+1][0] = max(dp[i+1][0], dp[i][1]-as[i])
		dp[i+1][1] = max(dp[i+1][1], dp[i][0]-as[i])
		dp[i+1][1] = max(dp[i+1][1], dp[i][1]+as[i])
	}

	ans := max(dp[n-1][0]+as[n-1], dp[n-1][1]-as[n-1])
	out(ans)
}
