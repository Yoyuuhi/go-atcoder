...
func main() {
	...
	graphDfs(edges, x-1, y-1, make(map[int]int, n), deque)
	...
	ansFront := ans.Front()
	for ansFront.Value.(int) != y {
		outwoln(ansFront.Value.(int))
		outwoln(" ")
		ansFront = ansFront.Next()
	}
	out(y)
}

func graphDfs(edges [][]edge, now int, t int, passed map[int]int, deque *list.List) {
	if solved {
		return
	}
	passed[now] = 1
	...
	for _, edge := range edges[now] {
		if passed[edge.to] != 1 {
			graphDfs(edges, edge.to, t, passed, deque)
		}
	}
	if deque.Back().Value.(int) == now {
