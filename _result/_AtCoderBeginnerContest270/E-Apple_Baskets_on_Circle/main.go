...
func main() {
	...
	memNumTurn := map[int]int{}
	left := 0
	sumTurn := 0
	for left < n {
		turn := tAs[left]
		if turn > 0 {
			memNumTurn[n-left] = turn - sumTurn
			sumTurn += turn - sumTurn
		}
		for left < n && turn == tAs[left] {
			left++
		}
	}

	nums := make([]int, len(memNumTurn))
	ind := 0
	for k, _ := range memNumTurn {
		nums[ind] = k
		ind++
	}
	sortir(nums)
	...
	left = 0
	sumTurn = 0
	for left < len(nums) && k >= nums[left]*memNumTurn[nums[left]] {
		k -= nums[left] * memNumTurn[nums[left]]
		sumTurn += memNumTurn[nums[left]]
		left++
	}
	var restTurn int
	var restNum int
	if left < len(nums) {
		restTurn = k / nums[left]
		restNum = k % nums[left]
	}

	for i, _ := range as {
		as[i] = max(0, as[i]-sumTurn-restTurn)
		if restNum > 0 && as[i] > 0 {
			as[i] -= 1
			restNum--
		}
